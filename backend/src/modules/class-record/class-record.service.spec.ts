...
describe('ClassRecordService', () => {
  ...
  let academicPolicyService: { assertAssessmentAction: jest.Mock };
  ...
  it('applies the class grading profile when generating class-record categories', async () => {
    db.query.classes.findFirst.mockResolvedValue({
      id: 'class-1',
      teacherId: 'teacher-1',
    ...
    await service.generateClassRecord(
      { classId: 'class-1', gradingPeriod: 'Q1' },
      'teacher-1',
      ['teacher'],
    );
  ...
  it('stores class teacher as class-record owner when generated by admin', async () => {
    ...
    const insertClassRecordValues = jest.fn().mockReturnValue({
      returning: jest.fn().mockResolvedValue([{ id: 'record-1' }]),
    });

    db.insert
      .mockReturnValueOnce({
        values: insertClassRecordValues,
      })
      .mockReturnValueOnce({
        values: jest.fn().mockReturnValue({
          returning: jest.fn().mockResolvedValue([{ id: 'cat-1' }]),
    ...
    expect(insertClassRecordValues).toHaveBeenCalledWith(
      expect.objectContaining({
        classId: 'class-1',
        teacherId: 'teacher-1',
        gradingPeriod: 'Q1',
  ...
  it('keeps score caps protected during Maintenance Access', async () => {
    academicPolicyService.assertAssessmentAction.mockResolvedValueOnce({
      maintenanceAccess: {
        maintenanceAccessVersion: 8,
        maintenanceAccessExpiresAt: '2026-09-12T05:00:00.000Z',
    ...
    await expect(
      service.bulkRecordScores(
        'item-1',
        { scores: [{ studentId: 'student-1', score: 11 }] },
        'admin-1',
        ['admin'],
      ),
    ).rejects.toThrow('between 0 and max score of 10');
