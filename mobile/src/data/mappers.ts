...
const subjectVisuals = [
  {
    match: ["math", "algebra", "geometry", "statistics"],
    emoji: "📐",
    color: colors.red,
    bgColor: colors.paleRed,
    gradient: gradients.assessments,
  },
  {
  ...
  {
    match: ["english", "reading", "literature"],
    emoji: "📚",
    color: colors.blue,
    bgColor: colors.paleBlue,
    gradient: gradients.classes,
  },
  {
    match: ["history", "social", "civics"],
    emoji: "🏛️",
    color: colors.orange,
...
export function getVisualForSubject(subjectName: string) {
  return (
    subjectVisuals.find((entry) =>
      entry.match.some((keyword) => normalize(subjectName).includes(keyword)),
    ) ?? subjectVisuals[0]
  );
}
...
export function toSubjectCard(
  ...
  const completedCount = safeCompletions.filter(
  ...
  return {
    ...
    progress:
      performance?.blendedScore != null
        ? Math.round(boundAcademicPercentage(performance.blendedScore))
        : progress,
    totalLessons,
...
export function toLessonCards(
  ...
  const ordered = [...safeLessons].sort(
  ...
  return ordered.map((lesson, index) => ({
    ...
    description:
      lesson.description ?? "Lesson content is available in the class module.",
...
export function findContinueLearning(
  subjects: SubjectCard[],
  lessonMap: Record<string, LessonCard[]>,
) {
  return subjects
    .flatMap((subject) =>
...
export function toAssessmentCard(
  ...
  return {
    ...
    status,
    score:
      latestAttempt?.scorePercent != null || latestAttempt?.score != null
        ? boundAcademicPercentage(
            latestAttempt.scorePercent ?? latestAttempt.score ?? 0,
          )
        : undefined,
...
export function toTutorRecommendationCards(
  playlist: PlaylistResponse | undefined,
  subject: SubjectCard | undefined,
): TutorRecommendationCard[] {
...
export function buildAchievements(
  ...
  const totalCompletedLessons = subjects.reduce(
  ...
  return [
    {
      ...
      earnedDate:
        totalCompletedLessons > 0
          ? "Derived from lesson completions"
          : undefined,
    },
    {
      id: "assessment-finisher",
      title: "Assessment Finisher",
      description: "Submit at least one assessment attempt",
      emoji: "📝",
      earned: completedAssessments.length > 0,
      earnedDate:
        completedAssessments.length > 0
          ? "Derived from submitted attempts"
          : undefined,
    },
    {
      id: "steady-climber",
      title: "Steady Climber",
      description: "Reach 60% average class progress",
      emoji: "📈",
      earned:
        subjects.length > 0 &&
        Math.round(
      ...
      earnedDate: "Derived from class progress",
    },
    {
      id: "lxp-runner",
      title: "LXP Runner",
      description: "Complete a remedial checkpoint",
      emoji: "🚀",
      earned: (playlist?.progress.checkpointsCompleted ?? 0) > 0,
      earnedDate:
        (playlist?.progress.checkpointsCompleted ?? 0) > 0
          ? "Derived from LXP checkpoints"
          : undefined,
    },
    {
      id: "risk-reducer",
      title: "Risk Reducer",
      description: "Keep your at-risk classes below two",
      emoji: "🛡️",
      earned: (performance?.overall.atRiskClasses ?? 0) < 2,
      earnedDate: "Derived from performance snapshot",
    },
  ];
}
...
export function toUserProfileSummary(
  ...
  const fullName =
    ...
    user?.email?.split("@")[0] ||
    "Student";
  const totalLessonsCompleted = subjects.reduce(
    (total, subject) => total + subject.completedLessons,
    0,
  );
  const completedAssessments = assessments.filter(
    ...
    completedAssessments.length > 0
      ? Math.round(
          completedAssessments.reduce(
            (total, assessment) =>
              total + boundAcademicPercentage(assessment.score ?? 0),
            0,
          ) / completedAssessments.length,
        )
      : Math.round(
          boundAcademicPercentage(
            performance?.overall.averageBlendedScore ?? 0,
          ),
        );
  const xp =
    totalLessonsCompleted * 45 +
    completedAssessments.length * 60 +
    (playlist?.progress.xpTotal ?? 0);
  ...
  return {
    ...
    streak:
      playlist?.progress.streakDays ??
      Math.max(1, subjects.filter((entry) => entry.progress > 0).length),
