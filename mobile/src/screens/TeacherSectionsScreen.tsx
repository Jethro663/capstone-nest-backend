...
import {
  useTeacherSections,
  useTeacherSectionPresentationUpdateMutation,
  useUploadSectionBannerMutation,
} from "../api/hooks";
...
export function TeacherSectionsScreen({ navigation }: Props) {
  ...
  const [customizingSection, setCustomizingSection] =
    useState<TeacherSection | null>(null);
  const sectionPresentationMutation =
    useTeacherSectionPresentationUpdateMutation();
  ...
  const handleSelectPreset = async (presetId: string) => {
    ...
    try {
      await sectionPresentationMutation.mutateAsync({
      ...
      Alert.alert("Unable to update section palette", toAppError(err).message);
    }
  };
  ...
  const handleUploadImage = async () => {
    ...
    try {
      ...
      if (!permission.granted) {
        Alert.alert(
          "Permission required",
          "Please allow access to your photos to upload a section image.",
        );
        return;
      }
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((section) => {
      const haystack =
        `${section.name} ${section.gradeLevel} ${section.schoolYear} ${section.roomNumber ?? ""}`.toLowerCase();
  ...
  return (
    ...
    >
      <TeacherContextStrip
        ...
        subtitle="Roster, room, and schedule access."
        ...
        {filtered.length ? (
          filtered.map((section) => (
            <TeacherSectionPresentationCard
              ...
              onOpen={() =>
                navigation.navigate("TeacherSectionDetail", {
                ...
                {CLASS_CARD_PRESETS.map((opt) => (
                  <Pressable
                    ...
                    style={{
                      ...
                      borderWidth:
                        !customizingSection?.cardBannerUrl &&
                        customizingSection?.cardPreset === opt.id
                          ? 2
                          : 0,
