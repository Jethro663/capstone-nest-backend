...
export function ProgressScreen(_: Props) {
  ...
  const profileSummary = toUserProfileSummary(
    user ?? null,
    null,
    subjects,
  ...
  return (
    <ScreenScroll
      refreshControl={
        <Refreshable
          refreshing={
            classesQuery.isRefetching || performanceQuery.isRefetching
          }
          onRefresh={() => {
            void Promise.all([
      ...
      >
        <View style={{ flexDirection: "row", gap: 10, marginTop: 18 }}>
          <StatCard
            ...
            iconColor="#A5D6A7"
            value={profileSummary.totalLessonsCompleted}
            label="Lessons"
            translucent
          />
          <StatCard
            icon="star"
            iconColor="#FFF176"
            value={`${profileSummary.averageScore}%`}
            label="Avg Score"
            translucent
          />
          <StatCard
      ...
      <View style={{ paddingHorizontal: 20, marginTop: 20, gap: 18 }}>
        {primaryError ? (
          <Card>
            ...
            >
              Progress data is partially unavailable
            </Text>
            <Text
              style={{
                marginTop: 6,
        ...
        <Card>
          ...
          {chartData.length === 0 ? (
            <EmptyState
              ...
              subtitle="Start lessons or assessments to populate this chart."
            />
          ) : (
            <>
        ...
        <Card>
          ...
          >
            ...
            <Pill
              label={
                performanceQuery.data?.overall.atRiskClasses
                  ? `${performanceQuery.data.overall.atRiskClasses} at risk`
                  : "Stable"
              }
              backgroundColor={
                performanceQuery.data?.overall.atRiskClasses
          ...
          <Text
            style={{ marginTop: 6, fontSize: 12, color: colors.textSecondary }}
          >
        ...
        <View style={{ marginBottom: 8 }}>
          <SectionTitle
            ...
            right={
              <Pill
                label={`${achievements.filter((entry) => entry.earned).length}/${achievements.length}`}
