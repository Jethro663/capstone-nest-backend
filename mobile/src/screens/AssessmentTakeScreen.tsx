...
function buildUploadRules(assessment?: {
  ...
  return {
    maxBytes: Math.min(
      assessment?.maxUploadSizeBytes ?? DEFAULT_UPLOAD_MAX_BYTES,
      DEFAULT_UPLOAD_MAX_BYTES,
    ),
    ...
    allowedMimeTypes: assessment?.allowedUploadMimeTypes?.length
      ? [
          ...assessment.allowedUploadMimeTypes,
          "application/zip",
...
function isImageFile(
  file?: { mimeType?: string | null; originalName?: string | null } | null,
) {
...
function TimerBadge({
  ...
  return (
    ...
    >
      <Text
        style={{
          color: warning ? theme.red : theme.text,
          fontSize: 12,
          fontWeight: "900",
        }}
      >
...
export function AssessmentTakeScreen({ route, navigation }: Props) {
  ...
  const [attemptResult, setAttemptResult] =
    useState<OngoingAttemptResult | null>(null);
  const [attemptPreparationFailed, setAttemptPreparationFailed] =
    useState(false);
  ...
  const strictMode = Boolean(
    attemptResult?.strictMode || assessment?.strictMode,
  );
  ...
  const timer = resolveAttemptTimer(
    {
      ...
      timeLimitMinutes:
        attemptResult?.timeLimitMinutes ?? assessment?.timeLimitMinutes,
  ...
  const isActiveQuestionAttempt = Boolean(
    !isFileUploadAssessment && attempt?.id && !attempt.isSubmitted,
  );
  ...
  const saveProgress = useCallback(
    async (options?: {
      ...
      const nextResponses = buildAssessmentResponses(
        questions,
      ...
      try {
        ...
        setAttemptResult((current) =>
          current ? { ...current, attempt: updated } : current,
        );
        if (updated.lastQuestionIndex !== undefined) {
          setCurrentIndex(
            resolveCurrentQuestionIndex(
              updated.lastQuestionIndex,
              questions.length,
            ),
          );
        }
  ...
  const registerViolation = useCallback(
    async (reason: string) => {
      ...
      if (updated?.isSubmitted || nextState.locked) {
        setError(
          "This attempt was locked after repeated anti-cheat violations.",
        );
      }
  ...
  const warnBeforeLeaving = useCallback(() => {
    Alert.alert(
      ...
      "Stay in Nexora until you submit. Leaving the app is recorded as an anti-cheat violation.",
    );
  }, []);
    ...
    const prepareAttempt = async () => {
      try {
        ...
        const ongoingAttempt =
          await assessmentsApi.getOngoingAttempt(assessmentId);
        const currentAttempt =
          ongoingAttempt ?? (await assessmentsApi.startAttempt(assessmentId));
        if (!active) return;
        ...
        setAnswers(
          restoreDraftResponses(currentAttempt?.attempt?.draftResponses),
        );
        setCurrentIndex(
          Math.max(0, currentAttempt?.attempt?.lastQuestionIndex ?? 0),
        );
        setStatus("");
      } catch (rawError) {
        if (!active) return;
  ...
  useEffect(() => {
    setCurrentIndex((current) =>
      resolveCurrentQuestionIndex(current, questions.length),
    );
  }, [questions.length]);
  ...
  useEffect(() => {
    if (!isActiveQuestionAttempt) return undefined;
  ...
  }, [isActiveQuestionAttempt, registerViolation]);
    ...
    const subscription = AppState.addEventListener("change", (nextState) => {
      ...
      void assessmentsApi
        ...
        .then((ongoing) => {
          ...
          setCurrentIndex(
            resolveCurrentQuestionIndex(
              ongoing.attempt.lastQuestionIndex,
              questions.length,
            ),
          );
          setSyncPaused(false);
  ...
  useEffect(() => {
    const beforeRemove =
      typeof navigation.addListener === "function"
        ? navigation.addListener("beforeRemove", (event) => {
            ...
            if (!isActiveQuestionAttempt) return;
    ...
    const backHandler = BackHandler.addEventListener(
      ...
      () => {
        ...
        if (!isActiveQuestionAttempt) return false;
        warnBeforeLeaving();
        return true;
      },
    );
  ...
  }, [isActiveQuestionAttempt, navigation, warnBeforeLeaving]);

  const handleBackPress = () => {
    if (isActiveQuestionAttempt) {
      warnBeforeLeaving();
      return;
    }
    navigation.goBack();
  ...
  const paused =
    syncPaused || timer.secondsRemaining === 0 || lockedByViolation;
    ...
    async (files: UploadFile[]) => {
      ...
      try {
        ...
        for (const file of files) {
          ...
          const uploaded = await assessmentsApi.uploadSubmissionFile(
            assessmentId,
            {
              uri: file.uri,
  ...
  const removeUploadedFile = useCallback(
    async (fileId: string) => {
      try {
        ...
        const updated = await assessmentsApi.removeSubmissionFile(
          assessmentId,
          fileId,
        );
  ...
  const submitAssessment = async (options?: {
    allowQuestionExpiry?: boolean;
  }) => {
    if (!attempt?.id) {
      setError("This attempt is not ready yet.");
      return;
    }
    const blockedForSubmission =
      syncPaused ||
      timer.secondsRemaining === 0 ||
      lockedByViolation ||
      (questionExpired && !options?.allowQuestionExpiry);
    if (blockedForSubmission) {
      setError(
        "The attempt is paused until the timer, network sync, or lock state is resolved.",
      );
      return;
    }
  ...
  useEffect(() => {
    ...
    handledQuestionDeadlineRef.current = deadlineAt;
    const action = resolveQuestionDeadlineAction(
      currentIndex,
      questions.length,
    );
    if (action === "submit") {
  ...
  useEffect(() => {
    ...
    setStatus(
      "Assessment time ended. Confirming submission with the server...",
    );
  ...
  const renderQuestion = () => {
    ...
    const prompt = stripRichText(
      currentQuestion.content || "Question content unavailable",
    );
      ...
      >
        ...
        <View style={{ padding: 16, position: "relative", zIndex: 1 }}>
          ...
          >
            ...
            >
              <Text
                style={{ color: theme.blue, fontSize: 11, fontWeight: "800" }}
              >
                Q{currentIndex + 1}
              </Text>
            </View>
          ...
          {currentQuestion.type === "dropdown" ? (
            <View style={{ marginTop: 16, gap: 10 }}>
              <Pressable
                ...
                onPress={() =>
                  setExpandedDropdownId((current) =>
                    current === currentQuestion.id ? null : currentQuestion.id,
                  )
                }
                style={{
                  borderRadius: 16,
              ...
              >
                <Text
                  style={{
                    flex: 1,
                    color: selectedDropdownOption ? theme.text : theme.muted,
                    fontSize: 13,
                    fontWeight: "700",
                  }}
                >
                  {selectedDropdownOption
                    ? stripRichText(selectedDropdownOption.text)
                    : "Select an answer"}
                </Text>
                ...
                <Pressable
                  ...
                  onPress={() => {
                    if (currentQuestion.type === "multiple_select") {
                      ...
                      setQuestionAnswer(currentQuestion.id, [
                        ...activeSelections,
                      ]);
                      return;
                    }
  ...
  const headerUploadActionLabel =
    attempt?.isSubmitted && !attempt?.isReturned
      ? "Unsubmit"
      : submitMutation.isPending
        ? "Submitting"
        : "Submit";
  const headerUploadActionVariant =
    attempt?.isSubmitted && !attempt?.isReturned ? "secondary" : "primary";
  ...
  return (
    ...
    >
      ...
      >
          ...
          >
            ...
            >
              <Pressable
                ...
                accessibilityLabel="Leave assessment"
              ...
              {isFileUploadAssessment ? (
                <HeaderActionButton
                  ...
                  onPress={() =>
                    void (attempt?.isSubmitted && !attempt?.isReturned
                      ? unsubmitUpload()
                      : submitAssessment())
                  }
                />
                ...
                <View style={{ alignItems: "flex-end", gap: 6 }}>
                  {timedQuestionsEnabled ? (
                    <TimerBadge
                      seconds={questionTimer.secondsRemaining}
                      source="question"
                    />
                  ) : null}
                  <TimerBadge
        ...
        {isActiveQuestionAttempt ? (
          <View
            testID="assessment-attempt-in-progress"
            style={{
              minHeight: 58,
        ...
        >
          {attemptPreparationFailed ? (
            ...
            >
              ...
              >
                {error ||
                  "The assessment attempt could not be prepared right now."}
              </Text>
            </Card>
          ) : null}
          ...
          {(paused || questionExpired) && !attemptPreparationFailed ? (
            ...
            >
              <Text
                style={{ color: theme.amber, fontSize: 12, fontWeight: "800" }}
              >
                {questionExpired
            ...
            >
              ...
              {assessment?.teacherAttachmentFile ? (
                ...
                >
                  ...
                  <Text
                    style={{ marginTop: 2, color: theme.muted, fontSize: 11 }}
                  >
                    {formatBytes(
                      assessment.teacherAttachmentFile.sizeBytes ?? 0,
                    )}{" "}
                    • {assessment.teacherAttachmentFile.mimeType || "file"}
                  </Text>
                  <View
                    style={{
                      marginTop: 10,
                    ...
                    >
                      ...
                      {attempt?.id ? (
                        ...
                        >
                          ...
                          {!attempt.isSubmitted ? (
                            <Pressable
                              onPress={() => void removeUploadedFile(file.id)}
                            >
                              <MaterialCommunityIcons
                                name="close"
                                size={16}
          ...
          {status ? (
            ...
            >
              <Text
                style={{ color: theme.blue, fontSize: 12, fontWeight: "800" }}
              >
                {status}
              </Text>
            </View>
          ...
          {!!error && !attemptPreparationFailed ? (
            ...
            >
              <Text
                style={{ color: theme.red, fontSize: 12, fontWeight: "800" }}
              >
                {peekAppError(error).message}
              </Text>
            </View>
          ) : null}
              ...
              <Pressable
                ...
                onPress={() => {
                  if (!canGoPrevious) {
                    Alert.alert(
                      "Locked",
                      "This attempt cannot move backward while strict or timed mode is active.",
                    );
                    return;
                  }
              ...
              <Pressable
                onPress={() => {
                  if (!canAdvance) {
                    Alert.alert(
                      "Answer required",
                      "Answer the current question before moving forward in strict mode.",
                    );
                    return;
                  }
                  if (canGoNext) {
