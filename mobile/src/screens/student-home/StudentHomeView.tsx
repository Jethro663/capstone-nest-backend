...
export function StudentHomeView({
  ...
  }, [continueLearning, recentLessons]);
  const agenda = useMemo(
    () =>
      buildStudentHomeAgenda({
        assessments: pendingAssessments.map((item) => ({
          ...item,
          id: item.assessment.id,
          dueAt: item.assessment.dueDate,
        })),
        schedule: todaySchedule.map((item) => ({
          ...item,
          startsAtMinutes: clockToMinutes(item.startTime),
        })),
        continueLearning: learningCandidates.map((item) => ({
          ...item,
          id: item.lesson.id,
        })),
      }),
    [learningCandidates, pendingAssessments, todaySchedule],
  );

  const nextAssessment =
    agenda.next?.kind === "assessment"
      ? agenda.dueSoon.find((item) => item.id === agenda.next?.id)
      : undefined;
  const nextLesson =
    agenda.next?.kind === "lesson"
      ? learningCandidates.find((item) => item.lesson.id === agenda.next?.id)
      : undefined;
  const nextClass =
    agenda.next?.kind === "schedule"
      ? agenda.today.find((item) => item.id === agenda.next?.id)
      : undefined;
  const secondaryAssessment = agenda.dueSoon.find(
    (item) => item.id !== nextAssessment?.assessment.id,
  );
  const secondaryLesson = learningCandidates.find(
    (item) => item.lesson.id !== nextLesson?.lesson.id,
  );

  const priorityTitle =
    nextAssessment?.assessment.title ??
    ...
    >
      ...
      <View style={styles.welcomeHero}>
        <View style={styles.welcomeCopy}>
          <Text style={styles.heroEyebrow}>Today at GABHS</Text>
        ...
        <Image
          testID="student-home-ja"
          accessibilityIgnoresInvertColors
          source={require("../../../assets/ja/ja_cheer.png")}
          style={styles.jaMascot}
        />
      </View>

      {errorMessage ? (
        <StudentInlineNotice
          title="Some home data could not load"
      ...
      <View style={styles.section}>
        <SectionHeading eyebrow="Start here" title="Up next" />
        <View
          testID="student-home-priority-surface"
        ...
        >
          <Pressable
            ...
            style={({ pressed }) => [
              styles.priorityPressTarget,
              pressed ? styles.priorityPressed : null,
            ]}
          >
      ...
      <View
        testID="student-home-section-divider"
            ...
            <View style={styles.emptyDay}>
              ...
              <View style={styles.scheduleCopy}>
                <Text style={styles.scheduleTitle}>
                  No classes on your schedule
                </Text>
                <Text style={styles.scheduleMeta}>
                  A good time to review or finish a task.
                </Text>
              </View>
            </View>
          )}
...
const styles = StyleSheet.create({
  ...
  priorityPressTarget: { minHeight: 166 },
