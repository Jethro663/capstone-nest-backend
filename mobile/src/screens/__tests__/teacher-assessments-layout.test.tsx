...
jest.mock("../../api/services/ai", () => ({
  aiApi: { deleteTeacherJob: jest.fn() },
}));
jest.mock("../../api/services/academic-state", () => ({
  academicStateService: { getCurrent: jest.fn() },
}));
jest.mock("../../api/teacher-ai-draft-jobs", () => ({
  clearTeacherAiDraftJobIdIfMatches: jest.fn(),
}));
jest.mock("../../components/teacher/TeacherConfirmModal", () => ({
  TeacherConfirmModal: () => null,
}));
jest.mock("../teacher-assessments/TeacherAiJobsPanel", () => {
  const ReactRuntime = require("react");
  return {
    TeacherAiJobsPanel: () =>
      ReactRuntime.createElement("Text", null, "AI Draft Jobs Surface"),
  };
});
...
jest.mock("../../components/teacher/TeacherMobilePrimitives", () => {
  ...
  return {
    ...
    TeacherRow: ({ title, subtitle, right, left }: any) =>
      ReactRuntime.createElement(
        "TeacherRow",
        null,
        left,
  ...
  return {
    TeacherContextStrip: ({ title, subtitle, status }: any) =>
      ReactRuntime.createElement(
        ...
        ReactRuntime.createElement(Text, null, title),
        ReactRuntime.createElement(Text, null, subtitle),
        ReactRuntime.createElement(Text, null, status),
      ),
    TeacherFlatSection: ({ title, subtitle, children }: any) =>
      ReactRuntime.createElement(
        "TeacherFlatSection",
        null,
        ReactRuntime.createElement(Text, null, title),
...
describe("teacher assessment workspace layout", () => {
  it("prioritizes class accordions, preserves empty-class creation, and leaves AI jobs last", () => {
    ...
    mockedUseTeacherAiJobs.mockReturnValue({
      data: [],
      isLoading: false,
      isError: false,
      isRefetching: false,
