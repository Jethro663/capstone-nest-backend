...
jest.mock("../../components/admin/AdminPaginatedList", () => {
  const React = require("react");
  return {
    AdminPaginatedList: ({ data, renderItem, header }) =>
      React.createElement(
        "View",
        {},
        header,
...
jest.mock("../../components/teacher/TeacherMobilePrimitives", () => {
  ...
  return {
    ...
    TeacherPanel: container,
    TeacherScreen: container,
    TeacherStats: () => null,
    TeacherEmpty: () => null,
    TeacherActionButton: ({ label, ...props }) =>
      React.createElement(
        "Pressable",
        props,
        React.createElement("Text", {}, label),
      ),
    TeacherChip: ({ label, ...props }) =>
      React.createElement(
        "Pressable",
        props,
        React.createElement("Text", {}, label),
...
jest.mock("../../components/admin/AdminMobilePrimitives", () => {
  ...
  return {
    ...
    AdminChip: ({ label, ...props }) =>
      React.createElement(
        "Pressable",
        props,
        React.createElement("Text", {}, label),
      ),
    AdminField: ({ label, value, onChangeText, ...props }) =>
...
jest.mock("@tanstack/react-query", () => ({
  useQuery: () => ({
    data: { data: [{ id: "class-1", subjectCode: "MATH" }] },
    refetch: jest.fn(),
  }),
  useInfiniteQuery: () => ({
    data: { pages: [{ data: [mockNotice], page: 1, totalPages: 1, total: 1 }] },
    refetch: jest.fn(),
...
const button = (root, text) =>
  root
    ...
    .find(
      (node) => textOf(node) === text || node.props.accessibilityLabel === text,
    );
function changeRichText(root, html) {
  ...
  expect(mockInject).toHaveBeenCalledWith(
    expect.stringContaining("a notebook"),
  );
  act(() =>
    web.props.onMessage({
      nativeEvent: { data: JSON.stringify({ type: "change", html }) },
    }),
  );
...
it("teacher edits an existing HTML announcement visually and saves formatted HTML", async () => {
  ...
  act(() => {
    renderer = TestRenderer.create(
      <TeacherAnnouncementEditorModal
        visible
        editingId={mockNotice.id}
        initialTitle={mockNotice.title}
        initialContent={mockNotice.content}
        initialPinned
        onSave={onSave}
        onClose={jest.fn()}
      />,
  ...
  expect(
    renderer.root
      ...
      .some((node) => /<\/?p>/.test(node.props.value ?? "")),
  ).toBe(false);
...
it("admin reads and edits formatted announcements with the same editor", async () => {
  ...
  act(() =>
    button(renderer.root, "View Class update announcement").props.onPress(),
  );
  expect(
    renderer.root
      .findAllByType("Text")
      .some(
        (node) =>
          ...
          textOf(node) === "a notebook",
      ),
  ).toBe(true);
  act(() =>
    button(renderer.root, "Close announcement details").props.onPress(),
  );
  ...
  expect(announcementsApi.update).toHaveBeenCalledWith(
    "class-1",
    "notice-1",
    expect.objectContaining({ content: html, isPinned: true }),
  );
