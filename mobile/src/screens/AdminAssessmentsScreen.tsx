...
export function AdminAssessmentsScreen({ navigation }: Props) {
  ...
  return (
    <TeacherScreen title="Assessment administration" workspaceLabel="Admin workspace" subtitle="Cross-class assessment inventory with teacher-equivalent detail, grading, analytics, and lifecycle controls." icon="clipboard-text-outline" refreshing={refreshing} onRefresh={() => void Promise.all([classes.refetch(), ...queries.map((query) => query.refetch())])}>
      ...
      <TeacherPanel title="Create assessment" subtitle="Choose the target class, then complete the guided assignment setup."><View style={{ padding: 14, flexDirection: "row", flexWrap: "wrap", gap: 8 }}>{(classes.data ?? []).map((entry) => <TeacherActionButton key={entry.id} label={`New in ${entry.subjectCode}`} icon="plus" tone="green" onPress={() => (navigation.getParent() as unknown as { navigate: (name: string, params?: unknown) => void })?.navigate("TeacherCreateAssessment", { classId: entry.id })} />)}</View></TeacherPanel>
