...
import {
  useTeacherClasses,
  useTeacherClassPresentationUpdateMutation,
  useUploadClassBannerMutation,
} from "../api/hooks";
...
function formatSchedule(classItem: ClassItem) {
  ...
  if (!schedule)
    return classItem.room ? `Room ${classItem.room}` : "Schedule TBA";
...
export function TeacherClassesScreen({ navigation }: Props) {
  ...
  const [customizingClass, setCustomizingClass] = useState<ClassItem | null>(
    null,
  );
  ...
  const handleSelectPreset = async (presetId: string) => {
    ...
    try {
      await classPresentationMutation.mutateAsync({
      ...
      Alert.alert("Unable to update class palette", toAppError(err).message);
    }
  };
  ...
  const handleUploadImage = async () => {
    ...
    try {
      ...
      if (!permission.granted) {
        Alert.alert(
          "Permission required",
          "Please allow access to your photos to upload a class image.",
        );
        return;
      }
  ...
  const handleRemoveImage = async () => {
    ...
    try {
      ...
      Alert.alert("Failed to remove image", toAppError(err).message);
    }
  };
  ...
  return (
    ...
    >
      <TeacherContextStrip
        ...
        subtitle="Open a class for lessons, assessments, announcements, and roster."
        status={`${filteredClasses.length} shown`}
        icon="book-open-variant-outline"
      />
      <TeacherSearch
        value={search}
          ...
          <TeacherEmpty
            ...
            subtitle={
              search.trim()
                ...
                : "Assigned teacher classes will appear here."
            }
                ...
                {CLASS_CARD_PRESETS.map((opt) => (
                  <Pressable
                    ...
                    style={{
                      ...
                      borderWidth:
                        !customizingClass?.cardBannerUrl &&
                        customizingClass?.cardPreset === opt.id
                          ? 2
                          : 0,
