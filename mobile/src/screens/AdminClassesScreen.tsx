...
type Props = BottomTabScreenProps<
  MainTabParamList,
  "Classes" | "AdminClasses" | "AdminSections"
>;
...
export function AdminClassesScreen({ navigation, route }: Props) {
  const [mode, setMode] = useState<ViewMode>(
    route.name === "AdminSections" ? "sections" : "classes",
  );
  ...
  const teachers = useQuery({
    ...
    queryFn: () => adminApi.getAllUsers({ role: "teacher", status: "ACTIVE" }),
  });
  ...
  const visibleClasses = useMemo(() => {
    ...
    return (classes.data ?? []).filter((entry) => {
      const matchesSearch =
        !needle ||
        `${entry.subjectCode} ${entry.subjectName} ${entry.section?.name ?? ""} ${entry.schoolYear}`
          .toLowerCase()
          .includes(needle);
      const matchesStatus =
        status === "all" ||
        (status === "active" ? entry.isActive : !entry.isActive);
  ...
  const visibleSections = useMemo(() => {
    ...
    return (sections.data?.data ?? []).filter((entry) => {
      const matchesSearch =
        !needle ||
        `${entry.name} ${entry.gradeLevel} ${entry.schoolYear} ${entry.adviser?.firstName ?? ""} ${entry.adviser?.lastName ?? ""}`
          .toLowerCase()
          .includes(needle);
      const matchesStatus =
        status === "all" ||
  ...
  const editClass = (entry: (typeof visibleClasses)[number]) => {
    ...
    setGrade(
      entry.subjectGradeLevel === "8" ||
        entry.subjectGradeLevel === "9" ||
        entry.subjectGradeLevel === "10"
    ...
    try {
      ...
      if (mode === "sections") {
        ...
        if (
          ...
          throw new Error(
            "Subject code, section, teacher, room, and at least one schedule day are required.",
          );
        }
        if (
          ...
          throw new Error(
            "Enter a valid schedule with the start time before the end time.",
          );
        }
        const payload = {
  ...
  const visibleCount =
    mode === "classes" ? visibleClasses.length : visibleSections.length;
    ...
    >
      <AdminMetricStrip
        items={[
          { label: "Classes", value: classes.data?.length ?? 0 },
          {
            label: "Sections",
            value:
              sections.data?.pagination?.total ??
      ...
      >
        <AdminChip
          label="Classes"
          active={mode === "classes"}
          onPress={() => selectMode("classes")}
        />
        <AdminChip
          label="Sections"
          active={mode === "sections"}
          onPress={() => selectMode("sections")}
        />
        <View style={{ flex: 1 }} />
        <AdminButton
          label={
            showCreate
              ? "Close"
              : `New ${mode === "classes" ? "class" : "section"}`
          }
      ...
      {showCreate ? (
        ...
        >
          ...
          >
            <AdminField
              label={mode === "classes" ? "Subject name" : "Section name"}
              value={name}
              onChangeText={setName}
              placeholder={mode === "classes" ? "Mathematics" : "Bonifacio"}
            />
            {mode === "classes" ? (
              <AdminField
                label="Subject code"
            ...
            {mode === "classes" ? (
              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6 }}>
                {(sections.data?.data ?? [])
                  .filter((entry) => entry.gradeLevel === grade)
                  .map((entry) => (
                    <AdminChip
                      key={entry.id}
                      label={`Section ${entry.name}`}
                      active={selectedSection === entry.id}
                      onPress={() => setSelectedSection(entry.id)}
                    />
                  ))}
              </View>
            ) : null}
            ...
            >
              ...
              <AdminButton
                label={busy ? "Saving…" : editingId ? "Save changes" : "Create"}
                icon="content-save"
                tone="green"
                variant="solid"
                disabled={busy}
      ...
      <AdminSection
        title={mode === "classes" ? "Class records" : "Section records"}
        subtitle={`${visibleCount} visible ${visibleCount === 1 ? "record" : "records"}`}
      >
        {mode === "classes"
          ? visibleClasses.map((entry) => (
              <AdminDataRow
                ...
                meta={
                  entry.schedules?.[0]
                    ? `${entry.schedules[0].days.join(", ")} · ${entry.schedules[0].startTime}-${entry.schedules[0].endTime}${entry.room ? ` · Room ${entry.room}` : ""}`
                    : entry.room
                      ? `Room ${entry.room}`
                      : "No schedule"
                }
                ...
                right={
                  <View style={{ gap: 4 }}>
                    <AdminButton
                      label="Edit"
                      variant="text"
                      onPress={() => editClass(entry)}
                    />
                    <AdminButton
                      label={entry.isActive ? "Archive" : "Permanently delete"}
                      tone={entry.isActive ? "amber" : "red"}
                      variant="text"
                      onPress={() =>
                        rootNavigation?.navigate("AdminLifecycleReview", {
                          targetType: "CLASS",
                          targetId: entry.id,
                          targetLabel: `${entry.subjectName} (${entry.subjectCode})`,
          ...
          : visibleSections.map((entry) => (
              <AdminDataRow
                ...
                meta={
                  entry.adviser
                    ? `${entry.adviser.firstName ?? ""} ${entry.adviser.lastName ?? ""}`.trim()
                    : "No adviser assigned"
                }
                status={entry.isActive ? "Active" : "Archived"}
                statusTone={entry.isActive ? "green" : "neutral"}
                onPress={() =>
                  rootNavigation?.navigate("TeacherSectionDetail", {
                    sectionId: entry.id,
                  })
                }
                right={
                    ...
                    <AdminButton
                      ...
                      onPress={() =>
                        rootNavigation?.navigate("AdminLifecycleReview", {
                          targetType: "SECTION",
                          targetId: entry.id,
                          targetLabel: `Grade ${entry.gradeLevel} · ${entry.name}`,
                          isActive: Boolean(entry.isActive),
                        })
                      }
                    />
          ...
          <AdminEmpty
            ...
            subtitle={
              hasFilters
                ...
                : `Create the first ${mode === "classes" ? "class" : "section"} above.`
            }
