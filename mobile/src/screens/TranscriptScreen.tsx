...
export function TranscriptScreen({ navigation }: Props) {
  ...
  const groupedRows = useMemo(() => {
    return rows.reduce<Record<string, typeof rows>>((accumulator, row) => {
      const schoolYear =
        row.class?.schoolYear ||
        row.section?.schoolYear ||
        "Unknown School Year";
  ...
  return (
    <StudentScreen
      ...
      onRefresh={() => void transcriptQuery.refetch()}
      rightAction={
        <ToneTag
          label={pluralize(totalRows, "enrollment", "enrollments")}
          tone="blue"
        />
      }
    >
      ...
      <View style={{ paddingHorizontal: 16, paddingTop: 18, gap: 10 }}>
        ...
        {rows.length === 0 ? (
          <DarkPanel>
            ...
            >
              Try another search or wait for your academic enrollment history to
              sync.
            </Text>
          </DarkPanel>
        ) : (
                  ...
                  >
                    ...
                    >
                      <View style={{ flex: 1, minWidth: 0 }}>
                        ...
                        >
                          {row.class?.subjectName || "Unlinked Subject"} (
                          {row.class?.subjectCode || "--"})
                        </Text>
                        <Text
                          style={{
                            marginTop: 5,
        ...
        <DarkPanel>
          ...
          >
            <Text style={{ flex: 1, color: theme.muted, fontSize: 12 }}>
              ...
              {pluralize(totalRows, "total row", "total rows")}
            </Text>
