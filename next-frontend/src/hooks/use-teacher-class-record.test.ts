...
beforeEach(() => {
  ...
  (classRecordService.refreshAnnualSummary as jest.Mock).mockResolvedValue({
    data: { policy, students: [] },
  });
});
it("reconciles the active annual policy before showing annual evidence", async () => {
  const { result } = renderHook(() => useTeacherClassRecord("class-1"));
  await waitFor(() => expect(result.current.spreadsheetStatus).toBe("ready"));
  await act(async () => {
    await result.current.loadAnnual();
  });
  expect(classRecordService.refreshAnnualSummary).toHaveBeenCalledWith(
    "class-1",
  );
  expect(result.current.annualSummary).toEqual({ policy, students: [] });
});
