...
export function assessmentSettingsSummary(settings: AiAssessmentSettings): string {
  const yesNo = (value: boolean | undefined) => value ? "On" : "Off";
  const description = (settings.description ?? "").replace(/<[^>]*>/g, " ").replace(/&nbsp;/g, " ").replace(/\s+/g, " ").trim();
  return [
