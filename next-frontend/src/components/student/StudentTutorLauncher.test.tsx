...
describe('StudentTutorLauncher', () => {
  ...
  it('starts collapsed and expands into quick actions', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Expand AI tutor launcher' }));
  ...
  it('navigates to student chatbot when open button is clicked', () => {
    render(<StudentTutorLauncher />);

    fireEvent.click(screen.getByRole('button', { name: 'Expand AI tutor launcher' }));
    fireEvent.click(screen.getByRole('button', { name: 'Open AI Tutor' }));

    expect(push).toHaveBeenCalledWith('/dashboard/student/chatbot');
  });
});
