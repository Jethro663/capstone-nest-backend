...
import type { SectionEventTag, SectionUpcomingEvent } from './types';

interface SectionsUpcomingEventsCardProps {
  events: SectionUpcomingEvent[];
  selectedDateKey: string;
  seeAllHref?: string;
}

const TAG_ACCENT_CLASS: Record<SectionEventTag, string> = {
...
const DATE_BADGE_CLASS: Record<SectionEventTag, string> = {
...
function formatTagLabel(tag: SectionEventTag) {
...
export function SectionsUpcomingEventsCard({
  events,
  selectedDateKey,
  seeAllHref = '/dashboard/teacher/announcements',
}: SectionsUpcomingEventsCardProps) {
    ...
    <article className="rounded-[1.4rem] border border-[#e1deeb] bg-[#fbfafe] p-4 shadow-[0_20px_34px_-28px_rgba(22,32,58,0.45)]">
      <header className="mb-3 flex items-center justify-between gap-3">
        <div className="inline-flex items-start gap-2.5">
          ...
          <div>
            ...
            <h3 className="text-[1.3rem] font-semibold tracking-tight text-[#11192f]">
      ...
      {visibleEvents.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-[#d7d3e4] bg-white px-4 py-6 text-center">
          <p className="text-sm font-semibold text-[#27304a]">No upcoming events yet.</p>
          <p className="mt-1 text-xs text-[#7280a0]">Announcements and deadlines will appear here.</p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="ml-1.5 flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-base font-semibold text-[#11192f]">{event.title}</p>
