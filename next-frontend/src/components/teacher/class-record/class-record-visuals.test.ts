...
describe("class-record visual helpers", () => {
  ...
  it("searches names in either order and matches the learner LRN", () => {
    ...
    expect(
      filterClassRecordStudents(rows, categories, "santos ana", "all"),
    ).toHaveLength(1);
    expect(
      filterClassRecordStudents(rows, categories, "1002", "all")[0]?.studentId,
    ).toBe("ben");
  });
  ...
  it("filters each evidence state without treating a recorded zero as missing", () => {
    const rows = [
      ...
      student({
        studentId: "excused-with-score",
        categories: categories.map((category, index) => ({
          categoryId: category.id,
          scores: [index === 0 ? 18 : 10],
          scoreStatuses: [
            index === 0 ? ("excused_with_score" as never) : "recorded",
          ],
          scoreReasons: [
            index === 0 ? "Winner of the Division Science Fair" : null,
          ],
          total: index === 0 ? 18 : 10,
          ps: index === 0 ? 90 : 50,
          ws: index === 0 ? 31.5 : 17.5,
        })),
      }),
    ...
    expect(
      filterClassRecordStudents(rows, categories, "", "missing").map(
        (row) => row.studentId,
      ),
    ).toEqual(["missing"]);
    expect(
      filterClassRecordStudents(rows, categories, "", "excused").map(
        (row) => row.studentId,
      ),
    ).toEqual(["excused", "excused-with-score"]);
    expect(
      filterClassRecordStudents(rows, categories, "", "eligibility").map(
        (row) => row.studentId,
      ),
    ).toEqual(["eligibility"]);
    expect(
      filterClassRecordStudents(rows, categories, "", "intervention").map(
        (row) => row.studentId,
      ),
    ).toEqual(["intervention"]);
    expect(
      filterClassRecordStudents(rows, categories, "", "needs_attention").map(
        (row) => row.studentId,
      ),
    ).toEqual(["missing", "eligibility", "intervention"]);
  });
