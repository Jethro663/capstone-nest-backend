...
it("renders policy periods, learner bands, missing evidence and intervention with visible labels", () => {
  ...
  (summary.students[0] as any).accountState = "archived";
  render(<AcademicAnnualSummary summary={summary} refresh={jest.fn()} />);

  expect(
    screen.getByRole("columnheader", { name: "Term 1" }),
  ).toBeInTheDocument();
  expect(
    screen.getByRole("columnheader", { name: "Term 2" }),
  ).toBeInTheDocument();
  expect(
    screen.getByRole("columnheader", { name: "Term 3" }),
  ).toBeInTheDocument();
  expect(
    screen.queryByRole("columnheader", { name: /Term 4|Q4/ }),
  ...
  expect(
    screen.getByRole("rowheader", { name: /Archived account/i }),
  ...
  expect(
    screen.getByText("Term 2 grade evidence is missing."),
  ).toBeInTheDocument();
  expect(
    screen.queryByRole("button", { name: "External grade" }),
  ).not.toBeInTheDocument();
  expect(
...
it("keeps administrator evidence controls and their audited form intact", () => {
  ...
  expect(
    screen.getByLabelText(/Verified source \/ register reference/i),
  ).toBeInTheDocument();
  expect(
    screen.getByLabelText(/Reason and verification notes/i),
  ).toBeInTheDocument();
