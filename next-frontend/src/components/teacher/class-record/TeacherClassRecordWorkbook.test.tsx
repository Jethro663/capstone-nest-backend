...
it('presents readable workbook metadata, policy-aware navigation and semantic grade labels', () => {
  ...
  expect(
    screen.getByRole('button', { name: 'Term 1' }),
  ).toHaveAttribute('aria-pressed', 'true');
  expect(screen.queryByRole('button', { name: /Q4|Term 4/ })).not.toBeInTheDocument();
...
it('defaults to comfortable density and persists a compact preference safely', async () => {
  const { unmount } = render(<TeacherClassRecordWorkbook state={createState()} />);
...
it('resets search and filters when the selected record changes', async () => {
  ...
  await waitFor(() =>
    expect(screen.getByRole('searchbox', { name: 'Search learners' })).toHaveValue(''),
  );
...
it('keeps readiness-gated finalization and visible provisional and legacy states', async () => {
  ...
  const provisional = screen.getByText(/Provisional · Incomplete/).closest('td');
  ...
  expect(screen.getByText(/Legacy unverified · Passed/).closest('td')).toHaveAttribute(
    'data-grade-status',
    'legacy',
  );
});
