...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const fallback = window.setTimeout(() => {
      if (
        !isDashboardRolePathAllowed(window.location.pathname, normalizedRole)
      ) {
  ...
  useEffect(() => {
    ...
    if (window.sessionStorage.getItem(ROLE_MISMATCH_NOTICE_KEY) !== "pending")
      return;
      ...
      <div className={`flex h-screen overflow-hidden ${shellClass}`}>
        ...
        <Sidebar
          ...
          onTeacherCollapseToggle={() =>
            setPersistedTeacherSidebarCollapsed(true)
          }
          isStudentCollapsed={studentSidebarCollapsed}
          onStudentCollapseToggle={() =>
            setPersistedStudentSidebarCollapsed(true)
          }
        />
