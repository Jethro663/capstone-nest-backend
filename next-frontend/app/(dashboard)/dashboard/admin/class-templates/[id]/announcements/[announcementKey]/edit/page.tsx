...
export default function AdminTemplateAnnouncementEditPage() {
  const params = useParams<{ id: string; announcementKey: string }>();
  const router = useRouter();
  const templateId = String(params?.id ?? '');
  const announcementKey = String(params?.announcementKey ?? '');
  ...
  const announcementIndex = useMemo(() => resolveIndexKey(announcementKey), [announcementKey]);
  ...
  useEffect(() => {
    if (loading || announcementIndex < 0) return;
  ...
  }, [loading, announcements, announcementIndex]);
  ...
  const handleSave = async () => {
    if (announcementIndex < 0 || !announcements[announcementIndex]) {
      toast.error('Announcement context is missing');
      return;
    }
    ...
    nextAnnouncements[announcementIndex] = {
      ...nextAnnouncements[announcementIndex],
      title: safeTitle,
      content: safeContent,
      isPinned,
  ...
  if (announcementIndex < 0 || !announcements[announcementIndex]) {
    return (
      <div className="rounded-2xl border border-[var(--admin-outline)] bg-white p-5 text-sm text-[var(--admin-text-muted)]">
        Announcement not found. Save the workspace first and open announcement studio again.
        <div className="mt-4">
  ...
  return (
    <TeacherPageShell
      ...
      description="Refine pinned updates and messaging before publishing this template."
      actions={(
        <>
      ...
      stats={(
        <>
          ...
          <TeacherStatCard label="Announcement" value={`${announcementIndex + 1}`} accent="teal" />
          <TeacherStatCard label="Pinned" value={isPinned ? 'Yes' : 'No'} accent="amber" />
          <TeacherStatCard label="Status" value="Draft" accent="rose" />
        </>
      )}
    >
      <TeacherSectionCard title="Announcement Details" description="Keep announcements concise and reusable across generated class sections.">
