...
function formatStudentName(
  enrollment: NonNullable<ClassItem["enrollments"]>[number],
) {
...
function countModuleFiles(module: ClassModule) {
  return module.sections.reduce(
    (sum, section) =>
      sum + section.items.filter((item) => item.itemType === "file").length,
    0,
  );
}
...
export default function AdminClassDetailPage() {
  ...
  const activeTab: WorkspaceTab = isWorkspaceTab(viewParam)
    ? viewParam
    : "modules";
  ...
  const [discussionThreads, setDiscussionThreads] = useState<
    DiscussionThreadSummary[]
  >([]);
  const [selectedDiscussionThreadId, setSelectedDiscussionThreadId] = useState<
    string | null
  >(null);
  ...
  const [lifecycleEnrollment, setLifecycleEnrollment] = useState<
    NonNullable<ClassItem["enrollments"]>[number] | null
  >(null);
  ...
  const [assessmentDescriptionDraft, setAssessmentDescriptionDraft] =
    useState("");
    ...
    try {
      ...
      const [
        ...
        classService
          ...
          .catch(() => ({
            data: [] as NonNullable<ClassItem["enrollments"]>,
            count: 0,
          })),
        discussionBoardService
          ...
          .catch(() => ({
            data: {
              items: [] as DiscussionThreadSummary[],
              total: 0,
              page: 1,
              limit: 50,
            },
      ...
      const enrollments =
        enrollmentsRes.data || classRes.data.enrollments || [];
      ...
      const nextThreads = sortDiscussionThreads(
        discussionThreadsRes.data.items || [],
      );
  ...
  useEffect(() => {
    setSelectedModuleIds((current) =>
      current.filter((id) => modules.some((module) => module.id === id)),
    );
  }, [modules]);
  ...
  useEffect(() => {
    setSelectedAssessmentIds((current) =>
      current.filter((id) =>
        assessments.some((assessment) => assessment.id === id),
      ),
    );
  }, [assessments]);
  ...
  const handleBulkModuleVisibility = (isVisible: boolean) => {
    ...
    setConfirmation({
      ...
      onConfirm: async () => {
        await executeControlledAction(
          `module-bulk-visibility-${isVisible}`,
          async () => {
            await Promise.all(
              ids.map((moduleId) =>
                moduleService.releaseCoreModule(moduleId, { isVisible }),
              ),
            );
            toast.success(
              isVisible
                ? "Selected modules released"
                : "Selected modules hidden",
            );
  ...
  const handleBulkDeleteModules = () => {
    ...
    setConfirmation({
      ...
      description:
        "This permanently removes the selected modules from the class.",
      ...
      onConfirm: async () => {
        await executeControlledAction(
          ...
          async () => {
            await Promise.all(
              ids.map((moduleId) => moduleService.delete(moduleId)),
            );
            setModules((current) =>
              current.filter((module) => !ids.includes(module.id)),
            );
            setSelectedModuleIds([]);
  ...
  const toggleAssessmentPublication = async (assessment: Assessment) => {
    await executeControlledAction(
      `assessment-publication-${assessment.id}`,
      async () => {
  ...
  const toggleSelectAllAssessments = () => {
    setSelectedAssessmentIds((current) =>
      current.length === assessments.length
        ? []
  ...
  const handleBulkAssessmentPublication = (isPublished: boolean) => {
    ...
    setConfirmation({
      title: isPublished
        ? "Publish selected assessments?"
        : "Move selected assessments to draft?",
      ...
      onConfirm: async () => {
        await executeControlledAction(
          `assessment-bulk-publication-${isPublished}`,
          async () => {
            await Promise.all(
              ids.map((assessmentId) =>
                assessmentService.releaseCore(assessmentId, { isPublished }),
              ),
            );
            toast.success(
              isPublished
                ? "Selected assessments published"
                : "Selected assessments moved to draft",
            );
  ...
  const handleBulkDeleteAssessments = () => {
    ...
    setConfirmation({
      ...
      description:
        "This permanently removes the selected assessments from the class.",
      confirmLabel: "Delete selected",
      tone: "danger",
      onConfirm: async () => {
        await executeControlledAction(
          ...
          async () => {
            await Promise.all(
              ids.map((assessmentId) => assessmentService.delete(assessmentId)),
            );
            setAssessments((current) =>
              current.filter((assessment) => !ids.includes(assessment.id)),
            );
            setSelectedAssessmentIds([]);
  ...
  const toggleAnnouncementVisibility = async (announcement: Announcement) => {
    await executeControlledAction(
      `announcement-visibility-${announcement.id}`,
      async () => {
  ...
  const handleDeleteAnnouncement = (announcementId: string) => {
    setConfirmation({
      ...
      description:
        "This permanently removes the announcement from the class feed.",
  ...
  const handleDeleteExtraction = (extractionId: string) => {
    setConfirmation({
      ...
      description:
        "This removes the extraction record from the class workspace.",
      confirmLabel: "Delete extraction",
      tone: "danger",
      onConfirm: async () => {
        await executeControlledAction(
          `extraction-delete-${extractionId}`,
          async () => {
  ...
  const handleDeleteDiscussionComment = (comment: DiscussionComment) => {
    ...
    setConfirmation({
      ...
      description:
        "This removes the selected reply from the discussion thread.",
  ...
  const classInfoLine = useMemo(
    () => buildClassInfoLine(classItem),
    [classItem],
  );
  const scheduleLine = useMemo(
    () => formatScheduleLine(classItem),
    [classItem],
  );
  ...
  const allAssessmentsSelected =
    ...
    selectedAssessmentIds.length === assessments.length;
  const calendarItems = useMemo(() => {
    const assessmentItems = assessments
      ...
      .map((assessment) => ({
        ...
        subtitle: assessment.isPublished
    ...
    const announcementItems = announcements
      .filter((announcement) =>
        Boolean(announcement.scheduledAt || announcement.createdAt),
      )
      .map((announcement) => ({
  ...
  return (
    <div className="teacher-class-workspace-wrap">
      <ClassWorkspaceShell
        ...
        heroActions={
          <div className="flex flex-wrap justify-end gap-2">
            ...
            >
              <Link href={`/dashboard/admin/classes/${classId}/edit`}>
                Edit Class
              </Link>
            </Button>
            <Button
            ...
            {classItem.isActive ? (
              ...
              <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-2 text-sm font-bold text-amber-800">
                Archived classes can only be purged from the Classes archive
                list.
              </div>
            )}
          </div>
            ...
            <div className="rounded-[1.5rem] border border-[var(--admin-outline)] bg-white p-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-[#607089]">
                    Keep class metadata and operational state aligned without
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                <label className="space-y-1.5">
                  <span className="text-xs font-black uppercase tracking-[0.12em] text-[#6a7a90]">
                    Room
                  </span>
                  <Input
                    value={roomDraft}
            ...
            <div className="rounded-[1.5rem] border border-[var(--admin-outline)] bg-white p-5">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                <article className="rounded-2xl border border-[#dce5f2] bg-[#f8fbff] p-4">
                  <p className="text-xs font-black uppercase tracking-[0.12em] text-[#6a7a90]">
                    Students
                  </p>
                  <p className="mt-2 text-2xl font-black text-[#14233d]">
                    {enrollments.length}
                  </p>
                </article>
                <article className="rounded-2xl border border-[#dce5f2] bg-[#f8fbff] p-4">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                >
                  <Link
                    href={`/dashboard/admin/classes/${classItem.id}/students/add`}
                  >
                    Add Class Students
                  </Link>
                </Button>
                <Button
                  ...
                  className="admin-button-outline rounded-xl font-black"
                >
                  <Link href={`/dashboard/admin/users/${classItem.teacherId}`}>
                    Teacher Profile
                  </Link>
                </Button>
              </div>
          ...
          <section className="teacher-class-workspace__panel">
            <div className="teacher-class-workspace__panel-head">
              <div>
                ...
                <p>
                  {modules.length} module{modules.length === 1 ? "" : "s"} with
                  direct release and order control.
                </p>
              </div>
                      ...
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div className="flex flex-1 items-start gap-4">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold text-[#607089]">
                              ...
                              <span>
                                {countModuleAssessments(module)} assessments
                              </span>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          <Button
                            ...
                            onClick={() =>
                              void moveModuleOneStep(module.id, -1)
                            }
                          >
                          ...
                          <Button
                            ...
                            className="teacher-class-workspace__outline"
                            disabled={
                              busyAction === `module-visibility-${module.id}`
                            }
            ...
            <div className="teacher-class-workspace__panel-head">
              <div>
                ...
                <p>
                  ...
                  {assessments.length === 1 ? "" : "s"} with publication
                  control.
                </p>
              </div>
              <div className="teacher-class-workspace__head-actions">
            ...
            <div className="space-y-4">
              {assessments.length === 0 ? (
                ...
                assessments.map((assessment) => {
                  const selected = selectedAssessmentIds.includes(
                    assessment.id,
                  );
                  return (
                    <article
                      ...
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          <Button
                            ...
                            onClick={() =>
                              void toggleAssessmentPublication(assessment)
                            }
                          >
          ...
          <section className="teacher-class-workspace__panel">
            <div className="teacher-class-workspace__panel-head">
              <div>
                ...
                <p>
                  Monitor AI extraction progress, quality flags, and cleanup
            ...
            <div className="space-y-4">
              {extractions.length === 0 ? (
                <div className="teacher-class-workspace__empty">
                  No extraction runs found for this class.
                </div>
              ) : (
                    ...
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                      <div className="flex-1">
                        ...
                        <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold text-[#607089]">
                          ...
                          <span>
                            {extraction.structuredContent?.sections.length ?? 0}{" "}
                            sections
                          </span>
                          <span>
                            Created {formatDateTime(extraction.createdAt)}
                          </span>
                        </div>
                      </div>
                      <div className="flex flex-wrap items-center gap-2">
          ...
          <section className="teacher-class-workspace__panel">
            <div className="teacher-class-workspace__panel-head">
              <div>
                ...
                <p>
                  Control bulletin visibility and create new admin notices
                  directly from the class workspace.
                </p>
              </div>
              <div className="teacher-class-workspace__head-actions">
                <Button
                    ...
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <Button
                          ...
                          onClick={() =>
                            void toggleAnnouncementVisibility(announcement)
                          }
                        >
          ...
          <section className="teacher-class-workspace__panel">
            <div className="teacher-class-workspace__panel-head">
              <div>
                ...
                <p>
                  Review thread state, lock or reopen discussion, and moderate
                  replies without leaving the class page.
                </p>
              </div>
            </div>
            <div className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
                ...
                {discussionThreads.length === 0 ? (
                  ...
                  discussionThreads.map((thread) => (
                    ...
                    >
                      ...
                      <h3 className="mt-3 text-base font-black text-[#12233f]">
                        {thread.title}
                      </h3>
                      <p className="mt-2 text-sm leading-6 text-[#607089]">
              ...
              <div className="rounded-[1.6rem] border border-[#dce5f2] bg-white p-5 shadow-[0_18px_36px_rgba(16,37,74,0.08)]">
                {!selectedDiscussionThread ? (
                  <div className="teacher-class-workspace__empty">
                    Select a thread to inspect comments and moderation actions.
                  </div>
                ) : (
                  <>
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                      <div>
                        ...
                        <p className="mt-2 text-sm leading-6 text-[#607089]">
                          {stripHtml(selectedDiscussionThread.bodyHtml) ||
                            "No thread content."}
                        </p>
                      </div>
                            ...
                            <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                              <div>
                                ...
                                <div className="mt-3 flex flex-wrap gap-2 text-xs font-semibold text-[#607089]">
                                  ...
                                  <span>
                                    {comment.reactions.total} reactions
                                  </span>
                                </div>
                              </div>
                              <div className="flex flex-wrap gap-2">
          ...
          <section className="teacher-class-workspace__panel">
            <div className="teacher-class-workspace__panel-head">
              <div>
                ...
                <p>
                  Oversight-first view of quarter workbooks and linked grading
            ...
            <div className="space-y-4">
              {classRecordSummary.length === 0 ? (
                <div className="teacher-class-workspace__empty">
                  No class record workbooks found for this class.
                </div>
              ) : (
                  ...
                  >
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                      ...
                      <div className="rounded-2xl border border-[#dce5f2] bg-[#f8fbff] px-4 py-3 text-sm text-[#51657f]">
                        Read-only oversight surface. Use linked class-record
                        tools for score entry or workbook exports.
                      </div>
                    </div>
                  </article>
                ))
          ...
          <section className="teacher-class-workspace__panel">
            <div className="teacher-class-workspace__panel-head">
              <div>
                ...
                <p>
                  Manage the current roster, open user profiles, and remove
                  enrollments from the class.
                </p>
              </div>
              <div className="teacher-class-workspace__head-actions">
                <Button
                  ...
                  >
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        ...
                        <div className="mt-2 flex flex-wrap gap-2 text-xs font-semibold text-[#607089]">
                          ...
                          <span>
                            {enrollment.student?.lrn ||
                              enrollment.student?.profile?.lrn ||
                              "No LRN"}
                          </span>
                        </div>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        ...
                        >
                          <Link
                            href={`/dashboard/admin/users/${enrollment.studentId}`}
                          >
                        ...
                        <Button
                          ...
                          variant="outline"
                          className="teacher-class-workspace__outline teacher-class-workspace__outline-danger"
                          onClick={() => setLifecycleEnrollment(enrollment)}
                        >
                          <Trash2 className="h-4 w-4" />
                          Remove
                        </Button>
          ...
          <section className="teacher-class-workspace__panel">
            <div className="teacher-class-workspace__panel-head">
              <div>
                ...
                <p>
                  Unified view of due dates and scheduled announcements attached
                  to this class.
                </p>
              </div>
              <div className="teacher-class-workspace__head-actions">
      ...
      <Dialog open={moduleDialogOpen} onOpenChange={setModuleDialogOpen}>
        <DialogContent variant="admin">
          <DialogHeader>
            ...
            <DialogDescription>
              Add a new module directly to this class workspace.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                ...
                onChange={(event) =>
                  setModuleDescriptionDraft(event.target.value)
                }
                rows={4}
              />
            </div>
      ...
      >
        <DialogContent variant="admin">
          <DialogHeader>
            ...
            <DialogDescription>
              Create a new assessment shell for this class.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(event) =>
                  setAssessmentTitleDraft(event.target.value)
                }
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Textarea
                ...
                onChange={(event) =>
                  setAssessmentDescriptionDraft(event.target.value)
                }
                rows={4}
              />
            </div>
      ...
      >
        <DialogContent variant="admin">
          <DialogHeader>
            ...
            <DialogDescription>
              Post a new announcement to this class feed.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(event) =>
                  setAnnouncementTitleDraft(event.target.value)
                }
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Textarea
                ...
                onChange={(event) =>
                  setAnnouncementContentDraft(event.target.value)
                }
                rows={5}
              />
            </div>
      ...
      >
        <DialogContent variant="admin">
          <DialogHeader>
            ...
            <DialogDescription>
              Send this reply into the moderation audit trail for follow-up.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="rounded-xl border border-[#dce5f2] bg-[#f8fbff] p-4 text-sm text-[#607089]">
              {reportDialogComment
                ? stripHtml(reportDialogComment.bodyHtml) || "Image-only reply"
                : "No comment selected"}
            </div>
            <div className="space-y-1.5">
              ...
              <select
                ...
                onChange={(event) =>
                  setDiscussionReportReason(
                    event.target.value as DiscussionCommentReportReason,
                  )
                }
              >
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => void handleSubmitDiscussionReport()}
              disabled={reportingDiscussionComment}
            >
      ...
      {lifecycleEnrollment && classItem ? (
        <AdminLifecycleDialog
          open
          onOpenChange={(open) => !open && setLifecycleEnrollment(null)}
          title="Resolve class membership"
          description="Choose the real academic outcome. The review preserves official evidence and shows an actionable alternative when a correction is not valid."
          targetLabel={formatStudentName(lifecycleEnrollment)}
          initialIntent="CORRECT_CLASS_ENROLLMENT"
          intents={[
            {
              value: "CORRECT_CLASS_ENROLLMENT",
              label: "Correct mistaken class enrollment",
              description:
                "Remove only this class membership when it was added by mistake.",
            },
            {
              value: "WITHDRAW",
              label: "Withdraw learner from the section",
              description:
                "Close all current memberships and preserve submitted work.",
            },
          ]}
          preview={async (intent) =>
            (
              await adminLifecycleService.previewStudent({
                studentId: lifecycleEnrollment.studentId,
                sectionId: classItem.sectionId,
                classId:
                  intent === "CORRECT_CLASS_ENROLLMENT" ? classId : undefined,
                resolution: intent as "CORRECT_CLASS_ENROLLMENT" | "WITHDRAW",
              ...
              current
                ? {
                    ...
                    enrollments: (current.enrollments || []).filter(
                      (entry) => entry.id !== lifecycleEnrollment.id,
                    ),
                  }
                : current,
            );
            toast.success("Student membership updated");
          }}
        />
      ) : null}
