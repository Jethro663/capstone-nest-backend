...
import type {
  ...
  ClassRecordSlotOverviewCategory,
...
const RESULT_RELEASE_OPTIONS: Array<{
...
}> = [
  {
    ...
    description:
      "Students can see their score as soon as they submit, but question-by-question review stays hidden.",
  },
  {
    value: "full_after_delay",
    title: "Full review after a delay",
    description:
      "Students can open the full answer review after the release delay has passed.",
  },
  {
    value: "full_with_hints_after_delay",
    title: "Full review with hints after a delay",
    description:
      "Students get guided study hints first, then the full answer review unlocks later.",
  },
] as const;
...
const PASSING_SCORE_OPTIONS = Array.from(
  { length: 51 },
  (_, index) => 50 + index,
);
...
function feedbackLevelToResultReleaseMode(
  level: FeedbackLevel,
): ResultReleaseMode {
...
function resultReleaseModeToFeedbackLevel(
...
function buildAssessmentEditorDraftFingerprint(
  draft: AssessmentEditorLocalDraft,
) {
...
function isAssessmentEditorLocalDraft(
  value: unknown,
): value is AssessmentEditorLocalDraft {
...
function readAssessmentEditorLocalDraft(
  assessmentId: string,
): AssessmentEditorLocalDraft | null {
  if (typeof window === "undefined") return null;
  const raw = window.localStorage.getItem(
    assessmentEditorDraftStorageKey(assessmentId),
  );
  if (!raw) return null;
...
function writeAssessmentEditorLocalDraft(
  assessmentId: string,
  draft: AssessmentEditorLocalDraft,
) {
...
function finalizeBoundedPositiveIntegerInput(
  value: string | number | null | undefined,
  max: number,
  fallback: number,
) {
  const sanitized = sanitizeBoundedPositiveIntegerInput(
    String(value ?? ""),
    max,
  );
...
function parseFillBlankSettings(conceptTags: string[]) {
  return {
    fillBlankSmartCaseInsensitive: !conceptTags.includes(
      FILL_BLANK_CASE_SENSITIVE_TAG,
    ),
    fillBlankExperimentalSmartMatch: conceptTags.includes(
      FILL_BLANK_EXPERIMENTAL_SMART_TAG,
    ),
  };
}
...
function buildFillBlankConceptTags(
  ...
  const passthroughTags = conceptTags.filter(
...
function normalizeQuestion(question: AssessmentQuestion): QuestionDraft {
  ...
  return {
    ...
    conceptTags,
    fillBlankSmartCaseInsensitive:
      fillBlankSettings.fillBlankSmartCaseInsensitive,
    fillBlankExperimentalSmartMatch:
      fillBlankSettings.fillBlankExperimentalSmartMatch,
...
function getQuestionValidationIssueTitles(
  ...
  if (!content) {
    issues.push({
      ...
      description:
        "Add the question body so students know what they need to answer.",
    });
  }
  ...
  if (!Number.isInteger(points) || points < 1) {
    issues.push({
      ...
      description:
        "Set at least 1 point so the assessment can score this item correctly.",
    });
  }
  ...
  if (isFillBlank) {
    ...
    if (answers.length === 0) {
      issues.push({
        ...
        description:
          "Fill-in-the-blank items need at least one accepted answer before students can take them.",
      });
    }
  ...
  if (question.options.length < 2) {
    issues.push({
      ...
      description:
        "Add at least two choices so the question can be shown properly to students.",
    });
  }
  ...
  if (question.options.some((option) => !optionHasAnswerContent(option))) {
    issues.push({
      ...
      description:
        "Remove blank options or add text or an image to every visible choice.",
    });
  }
  ...
  if (!question.options.some((option) => option.isCorrect)) {
    issues.push({
      title: `${issuePrefix} needs at least one correct answer`,
      description:
        "Mark the correct choice so the question can be scored accurately.",
    });
  }
...
function AssessmentEditorGuideShot({
  ...
  return (
    <div
      className={`assessment-editor__guide-shot assessment-editor__guide-shot--${screen}`}
      style={{ position: "relative" }}
    >
      <AssessmentEditorGuidePin
        lineSide="right"
        lineWidth="5.1rem"
        style={{ right: "2.25rem", top: "2.25rem" }}
      >
        {screen === "overview"
          ? "Top summary panel"
          : screen === "build"
      ...
      <div className="assessment-editor__guide-canvas">
        ...
        <div className="assessment-editor__header-tabbar assessment-editor__guide-tabbar">
          <span
            className={`assessment-editor__header-tab ${screen === "overview" ? "is-active" : ""}`}
          >
            Settings
          </span>
          <span
            className={`assessment-editor__header-tab ${screen === "delivery" || screen === "placement" ? "is-active" : ""}`}
          >
            Advanced
          </span>
          <span
            className={`assessment-editor__header-tab ${screen === "build" && fileUploadMode ? "is-active" : ""}`}
          >
            Rubric
          </span>
          <span
            className={`assessment-editor__header-tab ${screen === "review" ? "is-active" : ""}`}
          >
            Analytics
          </span>
        </div>
        ...
        {screen === "overview" ? (
          <div className="assessment-editor__guide-scene assessment-editor__guide-scene--overview">
            <AssessmentEditorGuidePin
              lineSide="left"
              lineWidth="5.5rem"
              style={{ left: "0.95rem", top: "2.15rem" }}
            >
              Title field
            </AssessmentEditorGuidePin>
            <AssessmentEditorGuidePin
            ...
            <div className="assessment-editor__guide-summary">
              <div>
                <small>Format</small>
                <strong>
                  {fileUploadMode ? "File upload" : "Question set"}
                </strong>
              </div>
              <div>
        ...
        {screen === "build" ? (
          <div className="assessment-editor__guide-scene assessment-editor__guide-scene--build">
            ...
            <AssessmentEditorGuidePin
              lineSide="right"
              lineWidth="5.5rem"
              style={{ right: "0.9rem", top: "6.45rem" }}
            >
        ...
        {screen === "delivery" ? (
          <div className="assessment-editor__guide-scene assessment-editor__guide-scene--delivery">
            ...
            <AssessmentEditorGuidePin
              lineSide="left"
              lineWidth="5.1rem"
              style={{ left: "0.85rem", top: "4.85rem" }}
            >
              Attempts and time
            </AssessmentEditorGuidePin>
            <AssessmentEditorGuidePin
              lineSide="right"
              lineWidth="4.6rem"
              style={{ right: "0.85rem", top: "7.15rem" }}
            >
              Result settings
            </AssessmentEditorGuidePin>
        ...
        {screen === "placement" ? (
          <div className="assessment-editor__guide-scene assessment-editor__guide-scene--placement">
            ...
            <AssessmentEditorGuidePin
              lineSide="left"
              lineWidth="4.6rem"
              style={{ left: "0.85rem", top: "4.95rem" }}
            >
              Select slot mode
            </AssessmentEditorGuidePin>
            <div className="assessment-editor__guide-form-grid">
        ...
        {screen === "review" ? (
          <div className="assessment-editor__guide-scene assessment-editor__guide-scene--review">
            <AssessmentEditorGuidePin
              lineSide="left"
              lineWidth="4.8rem"
              style={{ left: "0.85rem", top: "2.05rem" }}
            >
            ...
            <div className="assessment-editor__guide-checklist-preview">
              <div>
                ...
                <p>
                  Each missing setup item now shows as one clean checklist line
...
function AssessmentEditorGuidePin({
  ...
  return (
    ...
    >
      ...
      />
      <span className="sr-only">{lineSide}</span>
    </span>
  );
}
      ...
      {delayVisible ? (
        <div className="assessment-editor__field">
          ...
          <Input
            ...
            value={delayHours}
            onChange={(event) =>
              onDelayHoursChange(Math.max(0, Number(event.target.value) || 0))
            }
            disabled={disabled}
          />
        </div>
...
export default function AssessmentEditorPage() {
  ...
  const policyPeriods = assessment?.academicCapabilities?.periods ?? [];
  const [saveState, setSaveState] =
    useState<AssessmentComposerSaveState>("saved");
  const [previewEnabled, setPreviewEnabled] = useState(false);
  const [panelOpen, setPanelOpen] = useState(false);
  const [addQuestionDialogOpen, setAddQuestionDialogOpen] = useState(false);
  const [insertAfterQuestionIndex, setInsertAfterQuestionIndex] = useState<
    number | null
  >(null);
  ...
  const [resultReleaseMode, setResultReleaseMode] =
    useState<ResultReleaseMode>("score_immediately");
  ...
  const [timeLimitMinutes, setTimeLimitMinutes] = useState<string>(
    String(DEFAULT_TIME_LIMIT_MINUTES),
  );
  const [dueDate, setDueDate] = useState("");
  const [feedbackDelayHours, setFeedbackDelayHours] = useState(
    DEFAULT_RESULT_RELEASE_DELAY_HOURS,
  );

  const [category, setCategory] = useState<ClassRecordCategory | "">("");
  const [quarter, setQuarter] = useState<GradingPeriod | "">("");
  const [lockedSystemQuarter, setLockedSystemQuarter] =
  ...
  const [slotOverview, setSlotOverview] =
    useState<ClassRecordSlotOverview | null>(null);
  const [slotOverviewLoading, setSlotOverviewLoading] = useState(false);
  const [slotOverviewError, setSlotOverviewError] = useState<string | null>(
    null,
  );
  ...
  const [allowedUploadExtensions, setAllowedUploadExtensions] = useState<
    string[]
  >(getDefaultUploadExtensions);
  const [allowedUploadMimeTypes, setAllowedUploadMimeTypes] = useState<
    string[]
  >(getDefaultUploadMimeTypes);
  const [maxUploadSizeBytes, setMaxUploadSizeBytes] = useState<number>(
    100 * 1024 * 1024,
  );
  const [teacherAttachmentFile, setTeacherAttachmentFile] = useState<
    Assessment["teacherAttachmentFile"] | null
  >(null);
  const [uploadingTeacherAttachment, setUploadingTeacherAttachment] =
    useState(false);

  const [rubricCriteria, setRubricCriteria] = useState<RubricCriterion[]>([]);
  const [analytics, setAnalytics] = useState<QuestionAnalyticsResponse | null>(
    null,
  );
  ...
  const sectionRefs = useRef<
    Record<AssessmentSetupSectionId, HTMLElement | null>
  >({
    basics: null,
    content: null,
    delivery: null,
  ...
  const fetchAssessment = useCallback(async () => {
    ...
    try {
      ...
      const serverSnapshot: AssessmentEditorLocalDraft = {
        ...
        resultReleaseMode: feedbackLevelToResultReleaseMode(
          (data.feedbackLevel as FeedbackLevel) || "immediate",
        ),
        assessmentType: (data.type as AssessmentType) || "quiz",
        passingScore: normalizePassingScore(data.passingScore),
        maxAttempts: finalizeBoundedPositiveIntegerInput(
          data.maxAttempts,
          MAX_ATTEMPTS,
        ...
        feedbackDelayHours: Math.max(
          0,
          data.feedbackDelayHours ?? DEFAULT_RESULT_RELEASE_DELAY_HOURS,
        ),
        category: data.classRecordCategory || "",
        quarter: (data.quarter as GradingPeriod) || "",
        placementMode:
          (placement?.placementMode as AssessmentPlacementMode) || "automatic",
        ...
        questionTimeLimitSeconds:
          data.questionTimeLimitSeconds === null ||
          data.questionTimeLimitSeconds === undefined
            ? ""
        ...
        allowedUploadExtensions:
          ...
          data.allowedUploadExtensions.length > 0
            ? data.allowedUploadExtensions
            : getDefaultUploadExtensions(),
        allowedUploadMimeTypes:
      ...
      const serverFingerprint =
        buildAssessmentEditorDraftFingerprint(serverSnapshot);
      const cachedDraft =
        !requestedReadOnly && data.academicCapabilities?.canPrepare
          ? readAssessmentEditorLocalDraft(assessmentId)
          : null;
  ...
  }, [applyDraftSnapshot, assessmentId, requestedReadOnly]);
  ...
  useEffect(() => {
    ...
    const loadAnalytics = async () => {
      try {
        ...
        const response =
          await assessmentService.getQuestionAnalytics(assessmentId);
        if (!cancelled) {
  ...
  useEffect(() => {
    ...
    const loadSlots = async () => {
      try {
        ...
        setSlotOverviewError(null);
        const recordsResponse = await classRecordService.getByClass(
          assessment.classId,
        );
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    setSaveState((current) =>
      current === "saving" || current === "error" ? current : "saved",
    );
  ...
  const rubricIsBalanced =
    rubricCriteria.length > 0 && rubricTotalPoints === RUBRIC_TOTAL_POINTS;

  const updateRubricPoints = useCallback(
    (targetIndex: number, rawValue: string) => {
      setRubricCriteria((current) => {
        ...
        const allowedForCurrent = Math.max(
          RUBRIC_TOTAL_POINTS - otherPointsTotal,
          0,
        );
        ...
        return current.map((entry, entryIndex) =>
          entryIndex === targetIndex
            ? {
                ...entry,
                points: normalizeRubricPoints(rawValue, allowedForCurrent),
              }
            : entry,
        );
      });
  ...
  const selectedCategorySlots =
    useMemo<ClassRecordSlotOverviewCategory | null>(() => {
      if (!slotOverview) return null;
      return (
        slotOverview.categories.find((entry) => entry.key === category) || null
      );
  ...
  const setupIssues = useMemo<AssessmentSetupIssue[]>(() => {
    ...
    if (!title.trim()) {
      issues.push({
        ...
        description:
          "Give the activity a clear name so teachers and students can identify it immediately.",
        section: "basics",
        severity: "required",
    ...
    if (!description.trim()) {
      issues.push({
        ...
        description:
          "A short note helps first-time teachers remember the purpose and context of this assessment.",
        section: "basics",
    ...
    if (assessmentType === "file_upload") {
      if (!fileUploadInstructions.trim()) {
        issues.push({
          ...
          description:
            "Students need clear instructions before they can submit a file correctly.",
          section: "content",
          severity: "required",
      ...
      if (allowedUploadExtensions.length === 0) {
        issues.push({
          ...
          description:
            "Select at least one file group so the submission form knows what to accept.",
          section: "content",
          severity: "required",
          actionLabel: "Open build content",
        });
      ...
      if (questions.length === 0) {
        issues.push({
          ...
          description:
            "Question assessments need at least one item before students can take them.",
          section: "content",
          severity: "required",
          actionLabel: "Open build content",
        });
      ...
      questions.forEach((question, index) => {
        getQuestionValidationIssueTitles(question, index).forEach(
          (issue, issueIndex) => {
            issues.push({
    ...
    if (!dueDate) {
      issues.push({
        ...
        description:
          "A due date gives teachers and students a clear schedule for completion.",
        section: "delivery",
        severity: "recommended",
    ...
    if (!randomizeQuestions && !timedQuestionsEnabled && !strictMode) {
      issues.push({
        ...
        description:
          "Decide whether this assessment should randomize, time, or lock question navigation.",
        section: "delivery",
        severity: "recommended",
        actionLabel: "Open delivery rules",
      });
    ...
    const placementStarted = Boolean(category || selectedSlotId);
    ...
    if (placementRequired && !category) {
      issues.push({
        ...
        description:
          "Publishing this assessment requires a class record category so grades land in the right bucket.",
        section: "placement",
        severity: "required",
    ...
    if (placementRequired && !quarter) {
      issues.push({
        ...
        description:
          "Pick the grading period before this assessment is released to students.",
        section: "placement",
        severity: "required",
        actionLabel: "Open class record setup",
      });
    ...
    if (placementMode === "manual" && placementRequired && !selectedSlotId) {
      issues.push({
        ...
        description:
          "Manual placement needs a specific slot before the assessment can be published.",
        section: "placement",
        severity: "required",
        actionLabel: "Open class record setup",
      });
    ...
    if (publishReady && issues.some((issue) => issue.severity === "required")) {
      issues.push({
        ...
        description:
          "The publish state is on, but at least one required setup item still needs attention.",
        section: "review",
        severity: "required",
  ...
  const requiredSetupIssues = setupIssues.filter(
    (issue) => issue.severity === "required",
  );
  const publishBlocked =
    availability === "given" && requiredSetupIssues.length > 0;
  const publishReady =
    availability === "given" && requiredSetupIssues.length === 0;
    ...
    () => [
      {
        ...
        screen: "overview",
        reminder:
          "Open warning first, then use the question mark for the full setup walkthrough.",
        steps: [
          {
            action: "Check",
      ...
      },
      {
        ...
        description:
          "This stage is where teachers write questions, preview content, and switch to file upload mode.",
        screen: "build",
        reminder:
          "Click the question card body to edit, then use duplicate or delete only when needed.",
        steps: [
          {
            action: "Choose",
      ...
      },
      {
        ...
        description:
          "These controls shape timing, attempts, visibility, and how much freedom students get while answering.",
        screen: "delivery",
        reminder:
          "Open the Advanced tab when you need timing, attempt, timer, or feedback controls.",
        steps: [
          {
            action: "Open",
      ...
      },
      {
        ...
        description:
          "Publishing needs the grading category, quarter, and slot setup to be clear before scores should count.",
        screen: "placement",
        reminder:
          "Use the Advanced tab for class record placement, and use manual slot placement only when you know the exact record item.",
        steps: [
          {
            action: "Choose",
      ...
      },
      {
        ...
        description:
          "The final check lives in the header: review the warning list, preview if needed, then save with the correct publish state.",
        screen: "review",
        reminder:
          "Before leaving the page, review the warning dialog once and confirm the save state is healthy.",
        steps: [
          {
            action: "Open",
  ...
  const handleAddQuestion = (
    ...
    setQuestions((current) => {
      const insertAt =
        afterIndex === null
          ? current.length
          : Math.min(afterIndex + 1, current.length);
  ...
  const handleMoveQuestion = (questionId: string, direction: "up" | "down") => {
    ...
    setQuestions((current) =>
      reorderAssessmentComposerQuestions(current, fromIndex, toIndex),
    );
  };
  ...
  const handleDuplicateQuestion = (questionId: string) => {
    ...
    setQuestions((current) => {
      ...
      const duplicate = duplicateAssessmentComposerQuestion(
        current[sourceIndex],
      );
      const next = current.slice();
  ...
  const handleDeleteQuestion = (questionId: string) => {
    ...
    setConfirmation({
      ...
      tone: "danger",
      onConfirm: async () => {
        const nextState = deleteAssessmentComposerQuestion(
          questions,
          questionId,
          selectedQuestionId,
        );
  ...
  const handleSave = async () => {
    ...
    if (availability === "given" && quarterStatus !== "ready") {
      toast.error(
        "Verify the current quarter before publishing this assessment",
      );
      return;
    }
    ...
    if (
      ...
      toast.error(
        "Release is available only in the active period. Save period changes as a draft first.",
      );
      return;
    }
  ...
  const handleSaveRubric = async () => {
    ...
    try {
      ...
      const totalRubricPoints = normalized.reduce(
      ...
      const response = await assessmentService.reviewRubric(
        assessment.id,
        normalized,
      );
  ...
  const toggleGroup = (
    groupKey: (typeof FILE_UPLOAD_TYPE_GROUPS)[number]["key"],
  ) => {
    const group = FILE_UPLOAD_TYPE_GROUPS.find(
    ...
    const fullyEnabled = extensions.every((ext) =>
      allowedUploadExtensions.includes(ext),
    );
    if (fullyEnabled) {
    ...
    setAllowedUploadExtensions((current) =>
      Array.from(new Set([...current, ...extensions])),
    );
    setAllowedUploadMimeTypes((current) =>
      Array.from(new Set([...current, ...mimeTypes])),
    );
  };
  ...
  const setupSections: Array<{
  ...
  }> = [
    {
      ...
      description:
        "Name the activity, choose the mode, and decide whether it stays draft or goes live.",
    },
    {
      id: "content",
      title: "Build content",
      description:
        "Write the learner-facing questions or configure the file-upload activity.",
    },
    {
      id: "delivery",
      title: "Delivery rules",
      description:
        "Set time, attempts, feedback timing, and student control rules.",
    },
    {
      id: "placement",
      title: "Class record setup",
      description:
        "Connect the activity to the correct grading category, quarter, and slot.",
    },
    {
      id: "review",
      title: "Final review",
      description:
        "Check readiness, preview the student flow, and save with confidence.",
    },
  ];
      ...
      <div className="assessment-editor__rubric-head">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Use this for file uploads or scored performance tasks.
          </p>
        </div>
        <div
            ...
            {rubricCriteria.length === 0 ? (
              ...
              rubricCriteria.slice(0, 3).map((criterion, index) => (
                <span key={criterion.id || index}>
                  {(criterion.title || `Criterion ${index + 1}`).trim()} ·{" "}
                  {Number(criterion.points) || 0}
                </span>
              ))
            )}
            {rubricCriteria.length > 3 ? (
              <span>+{rubricCriteria.length - 3} more</span>
            ) : null}
          </div>
        </div>
      </div>
            ...
            >
              ...
              <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_92px_auto]">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    updateRubricPoints(index, event.target.value)
                  }
                  placeholder="Points"
                  className="h-11 rounded-2xl border-slate-200 bg-white"
                />
  ...
  const analyticsContent = analyticsLoading ? (
    ...
    <div className="space-y-4 rounded-[1.5rem] border border-slate-200/80 bg-white p-4">
      <div>
        ...
        <p className="text-sm text-slate-500">
          Review response trends without leaving the composer.
        </p>
      </div>
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-4 rounded-[1.5rem] border border-slate-200/80 bg-white p-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Keep the high-level setup in one place while you build the
            assessment body on the page.
          </p>
        </div>
      ...
      {assessmentType === "file_upload" ? (
        ...
        >
          <div>
            ...
            <p className="text-sm text-slate-500">
              Choose where file upload scores should land before the assessment
              is published.
            </p>
          </div>
          ...
          <div className="assessment-editor__advanced-inline">
            <div className="assessment-editor__field">
              ...
              <select
                value={category}
                onChange={(event) =>
                  setCategory(event.target.value as ClassRecordCategory)
                }
            ...
            <div className="assessment-editor__field">
              ...
              <select
                ...
                value={quarter}
                onChange={(event) =>
                  setQuarter(event.target.value as GradingPeriod)
                }
                disabled={
                  isReadOnlyMode ||
                  quarterStatus !== "ready" ||
                  !policyPeriods.length
                }
              >
                <option value="">Select period</option>
                {quarter && !policyPeriods.some((p) => p.key === quarter) && (
                  <option value={quarter}>
                    Historical {quarter} — admin repair required
                  </option>
                )}
          ...
          {lockedSystemQuarter ? (
            <p className="text-xs text-slate-500">
              Active period:{" "}
              {policyPeriods.find((p) => p.key === lockedSystemQuarter)
                ?.label ?? lockedSystemQuarter}
              . Future periods support draft preparation; release waits for
              activation.
            </p>
          ) : null}
          ...
          {!quarter ? (
            ...
            <div className="assessment-editor__slots-grid">
              {(selectedCategorySlots?.slots ?? []).map((slot) => (
                <button
                  ...
                  onClick={() => {
                    if (placementMode !== "manual" || !slot.isSelectable)
                      return;
            ...
            <p className="assessment-editor__empty-small">
              No slots found for selected category.
            </p>
          )}
        </section>
      ) : null}
        ...
        {assessmentType === "file_upload" ? (
          <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
            <p className="font-semibold text-slate-900">
              Score is always 100 for file upload assessments.
            </p>
            <p className="mt-1">
              {rubricCriteria.length === 0
          ...
          <>
            <div className="assessment-editor__field">
              ...
              <Input
                ...
                onChange={(event) =>
                  setTimeLimitMinutes(
                    sanitizeBoundedPositiveIntegerInput(
                      event.target.value,
                      MAX_TIME_LIMIT_MINUTES,
                    ),
                  )
            ...
            <div className="assessment-editor__field">
              ...
              <select
                ...
                onChange={(event) =>
                  setPassingScore(
                    normalizePassingScore(Number(event.target.value)),
                  )
                }
                disabled={isReadOnlyMode}
              >
            ...
            {timedQuestionsEnabled ? (
              <div className="assessment-editor__field">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setQuestionTimeLimitSeconds(event.target.value)
                  }
                  disabled={isReadOnlyMode}
                />
              </div>
      ...
      {assessmentType !== "file_upload" ? (
        ...
        >
          <div>
            ...
            <p className="text-sm text-slate-500">
              Choose where scores should land before the assessment is
              published.
            </p>
          </div>
  ...
  const quarterContextLabel =
    quarterStatus === "loading"
      ...
      : quarterStatus === "error"
        ? "Quarter unverified"
        : lockedSystemQuarter
          ? `${policyPeriods.find((p) => p.key === quarter)?.label ?? quarter} · active ${policyPeriods.find((p) => p.key === lockedSystemQuarter)?.label ?? lockedSystemQuarter}`
          : "Quarter unavailable";
  ...
  const warningButtonLabel =
    setupIssues.length === 1
      ? "View 1 setup issue"
      : `View ${setupIssues.length} setup issues`;
  const visibleWarningLabel =
    setupIssues.length === 1
      ? "1 setup issue"
      : `${setupIssues.length} setup issues`;
  const panelTitle =
    rightTab === "settings"
      ? "Settings"
      : rightTab === "advanced"
      ...
      <article className="assessment-editor__inline-card assessment-editor__file-mode">
        ...
        <div className="assessment-editor__file-groups">
          {FILE_UPLOAD_TYPE_GROUPS.map((group) => {
            const enabled = group.extensions.every((ext) =>
              allowedUploadExtensions.includes(ext),
            );
            return (
              <label
                key={group.key}
                className="assessment-editor__file-group"
                data-active={enabled}
              >
                <input
                  type="checkbox"
                ...
                />
                <span
                  className="assessment-editor__file-group-indicator"
                  aria-hidden="true"
                >
                  {enabled ? "✓" : ""}
                </span>
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
          <div className="assessment-editor__field">
            ...
            <Input
              ...
              onChange={(event) => {
                ...
                setMaxUploadSizeBytes(
                  Math.min(Math.max(mb, 1), 100) * 1024 * 1024,
                );
              }}
              disabled={isReadOnlyMode}
            />
          ...
          <div className="assessment-editor__upload-policy">
            <p className="assessment-editor__upload-policy-title">
              Upload policy
            </p>
            ...
            <p className="assessment-editor__upload-policy-line">
              <strong>Reference file:</strong> Optional preview material for
              students before they submit.
            </p>
          </div>
        </div>
        ...
        <div className="assessment-editor__file-attachment">
          <div>
            ...
            <span>
              Upload an optional handout, sample, or guide that students can
              preview.
            </span>
          </div>
  ...
  return (
    <div className="assessment-editor assessment-editor--flattened px-4 pb-8 pt-2 lg:px-6">
      <header className="assessment-editor__header assessment-editor__header--sticky assessment-editor__workspace-header">
        <div className="assessment-editor__workspace-main">
          <div className="assessment-editor__workspace-title-block">
            ...
            <div
              className="assessment-editor__workspace-context"
              aria-label="Assessment context"
            >
              <span>
                {assessment.isCoreTemplateAsset
                  ? "Core template assessment"
                  : "Class assessment"}
              </span>
          ...
          <div
            className="assessment-editor__header-tabbar"
            role="tablist"
            aria-label="Assessment editor tools"
          >
            <Button
              type="button"
              variant="outline"
        ...
        <div className="assessment-editor__workspace-side">
          <div className="assessment-editor__header-helper-group">
            <div
              className="assessment-editor__header-score"
              aria-label={`Total score ${totalPoints} points`}
            >
          ...
          >
            <span
              className="assessment-editor__workbar-meta"
        ...
        >
          <div>
            <p className="text-sm font-semibold">
              Current quarter could not be verified
            </p>
            <p className="mt-1 text-sm text-amber-800">
              Drafts can still be saved, but publishing stays unavailable until
              the check succeeds.
            </p>
          </div>
          <Button
              ...
              {ASSESSMENT_COMPOSER_QUESTION_TYPES.map((entry) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      handleAddQuestion(entry.type, insertAfterQuestionIndex)
                    }
      ...
      <Dialog open={questionDetailsOpen} onOpenChange={setQuestionDetailsOpen}>
        <DialogContent className="max-w-3xl rounded-3xl border-slate-200 p-0">
          <DialogHeader className="border-b border-slate-100 px-6 py-5">
            ...
            <DialogDescription className="text-slate-500">
              Keep explanation text here so the main question editor stays
              focused on the prompt, choices, and images.
            </DialogDescription>
          </DialogHeader>
            ...
            {selectedQuestion ? (
              <div className="space-y-4">
                <div className="rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3">
                  ...
                  <RichTextRenderer
                    html={
                      selectedQuestion.content || "<p>Untitled question</p>"
                    }
      ...
      <div
        className={`fixed inset-0 z-40 transition ${panelOpen ? "pointer-events-auto" : "pointer-events-none"}`}
      >
        <button
          type="button"
        ...
        <DialogContent className="teacher-intervention-workspace__manual-dialog assessment-editor__manual-dialog">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              the assessment editor being explained.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => {
                ...
                setHelpPage((current) =>
                  Math.min(current + 1, guidePages.length - 1),
                );
              }}
            >
              Next page
      ...
      <Dialog open={warningOpen} onOpenChange={setWarningOpen}>
        <DialogContent className="assessment-editor__warning-dialog">
          <DialogHeader>
            ...
            <DialogDescription>
              Use this checklist to see what is still missing and jump straight
              to the part of the page that needs attention.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="assessment-editor__warning-summary-line">
            ...
            <span>
              <strong>{setupIssues.length - requiredSetupIssues.length}</strong>{" "}
              recommended
            </span>
          </div>
          ...
          <div className="assessment-editor__warning-groups assessment-editor__warning-groups--plain">
            {setupSections.map((section) =>
              groupedSetupIssues[section.id].length > 0 ? (
                <section
                  key={section.id}
                  className="assessment-editor__warning-group"
                >
                  <div className="assessment-editor__warning-group-head">
                    <p className="assessment-editor__kicker">{section.title}</p>
                    <span>
                      {groupedSetupIssues[section.id].length} item
                      {groupedSetupIssues[section.id].length === 1 ? "" : "s"}
                    </span>
                  </div>
                  <ol className="assessment-editor__warning-list" role="list">
                    {groupedSetupIssues[section.id].map((issue) => (
                      <li
                        key={issue.id}
                        className="assessment-editor__warning-line"
                      ...
                      >
                        <button
                          ...
                          aria-label={
                            issue.actionLabel || `Open ${section.title}`
                          }
                        >
                          <span
                            className="assessment-editor__warning-mark"
                            aria-hidden="true"
                          >
