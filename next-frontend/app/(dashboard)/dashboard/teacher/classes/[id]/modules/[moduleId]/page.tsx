...
const ATTACH_BLOCK_OPTIONS: Array<{
  ...
  },
  {
    ...
    label: "Assessment",
    description:
      "Attach assessment content and control student release with Give.",
    icon: ClipboardList,
    tone: "assessment",
  },
  {
...
const ASSESSMENT_ATTACH_MODE_OPTIONS: Array<{
...
}> = [
  {
    ...
    description:
      "Set the format, class-record placement, name, and schedule before opening the editor.",
    icon: ClipboardList,
  },
  {
...
const teacherModuleGuidePages: Array<{
...
}> = [
  {
    ...
    screen: "header",
    reminder:
      "Keep this header visible while jumping across sections and settings for this module.",
    steps: [
      {
        action: "Read",
  ...
  },
  {
    ...
    screen: "sections",
    reminder:
      "Template modules keep their copied structure, but teachers can still control student release and readiness here.",
    steps: [
      {
        action: "Create",
  ...
  },
  {
    ...
    description:
      "Use Add Block on a section to attach lesson, assessment, PDF, or PowerPoint blocks.",
    screen: "blocks",
    reminder:
      "Keep section blocks consistent: lessons, assessments, then references for a readable learner flow.",
    steps: [
      {
        action: "Add",
  ...
  },
  {
    ...
    screen: "locking",
    reminder:
      "Use locking for staged release and readiness checks before assessments open.",
    steps: [
      {
        action: "Unlock",
  ...
  },
  {
    ...
    description:
      "Notes help you keep reminders for this module without affecting learner-facing content.",
    screen: "notes",
...
function TeacherModuleGuideScreenshot({
  screen,
}: {
  screen: TeacherModuleGuideScreen;
}) {
        ...
        <div className="rounded-xl border border-[#1d3659] bg-[#10254a] p-4 text-white">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div>
              ...
              <p className="mt-2 text-sm text-[#b6c8df]">
                Science 7 • Week 1 • 2 lessons • 1 assessment
              </p>
            </div>
            <button
              type="button"
        ...
        <div className="rounded-xl border border-[#e4ecf4] bg-white p-4">
          ...
          <div className="mt-3 rounded-lg border border-[#e2e9f4] bg-[#f8fbfe] p-3">
            ...
            <p className="mt-1 text-sm text-[#60789a]">
              Drag by handle to reorder sections.
            </p>
          </div>
        </div>
      ...
      >
        <div className="rounded-xl border border-[#d8d5cf] bg-white p-4">
          ...
          <div className="mt-2 rounded-lg border border-[#e2dad3] bg-[#fffdfa] p-3">
            <div className="mb-2 flex items-center justify-between">
              <span className="text-sm font-black text-[#143155]">
    ...
    return (
      ...
      >
        <div className="rounded-xl border border-[#e4ecf4] bg-white p-4">
          ...
          <p className="mt-3 text-xs text-[#5f7698]">
...
function titleForItem(item: ModuleItem) {
  ...
  if (item.itemType === "assessment")
    return item.assessment?.title || "Untitled assessment";
...
function itemMeta(item: ModuleItem) {
  if (item.itemType === "assessment") {
    const scoreText = item.assessment?.totalPoints
      ? `${item.assessment.totalPoints} pts`
      : "Assessment";
    return scoreText;
  }
...
export default function TeacherModuleDetailPage() {
  ...
  const [attachState, setAttachState] =
    useState<AttachState>(INITIAL_ATTACH_STATE);
  ...
  const [assignmentWizardSectionId, setAssignmentWizardSectionId] = useState<
    string | null
  >(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const [classResponse, modulesResponse, assessmentResponse] =
        await Promise.all([
  ...
  const activeGuidePage =
    teacherModuleGuidePages[helpPage] ?? teacherModuleGuidePages[0];
  ...
  useEffect(() => {
    ...
    setAttachState((current) => {
      if (
        current.itemId &&
        availableAttachOptions.some((option) => option.id === current.itemId)
      ) {
        return current;
      }
      return {
    ...
    attachState.assessmentMode,
    attachState.itemType,
    attachState.open,
    availableAttachOptions,
  ]);
  ...
  const runModulePatch = async (patch: {
    isVisible?: boolean;
    isLocked?: boolean;
    teacherNotes?: string;
  }) => {
  ...
  const handleReleaseCoreModule = async (patch: {
    isVisible?: boolean;
    isLocked?: boolean;
  }) => {
    ...
    try {
      ...
      if (patch.isLocked !== undefined) {
        toast.success(
          patch.isLocked ? "Default module locked" : "Default module unlocked",
        );
      } else {
        toast.success(
          patch.isVisible
            ? "Default module released to students"
            : "Default module hidden from students",
        );
      }
    } catch {
      setModule(previous);
  ...
  const confirmDeleteSection = (sectionId: string) => {
    setConfirmation({
      ...
      description:
        "The section and all attached items will be removed from this module.",
      tone: "danger",
  ...
  const handleReorderSections = async (targetSectionId: string) => {
    if (!module || !draggingSectionId || draggingSectionId === targetSectionId)
      return;
    ...
    const moved = moveEntry(sections, fromIndex, toIndex).map(
      (section, index) => ({
        ...section,
        order: index + 1,
      }),
    ...
    setModule((current) =>
      current ? { ...current, sections: moved } : current,
    );
    ...
    try {
      ...
      setModule((current) =>
        current ? { ...current, sections: previous } : current,
      );
  ...
  const handleReorderItems = async (
    ...
    if (!module || !draggingItem || draggingItem.sectionId !== sectionId)
      return;
    ...
    const fromIndex = section.items.findIndex(
      (item) => item.id === draggingItem.itemId,
    );
    const toIndex = section.items.findIndex((item) => item.id === targetItemId);
    if (fromIndex < 0 || toIndex < 0) return;

    const reorderedItems = moveEntry(section.items, fromIndex, toIndex).map(
      (item, index) => ({
        ...item,
        order: index + 1,
      }),
  ...
  const confirmDetachItem = (itemId: string) => {
    setConfirmation({
      ...
      details:
        "The source lesson or assessment remains available in the class.",
      onConfirm: async () => {
  ...
  const handleAttachItem = async () => {
    if (!attachState.sectionId || !attachState.itemType || attachingItem)
      return;
    ...
    if (
      ...
      if (!user?.id) {
        toast.error(
          "Your teacher session could not be verified. Refresh and try again.",
        );
        return;
      }
      const sectionId = attachState.sectionId;
      resetAttachWorkflow();
      setAssignmentWizardSectionId(sectionId);
      return;
    }
      ...
      if (attachState.itemType === "lesson") {
        ...
        if (attachSource === "library") {
          ...
          payload = {
            ...
            metadata: {
              fileSubtype:
                uploadedSubtype === "file"
                  ? getFileSubtype(attachState.file)
                  : uploadedSubtype,
            },
          };
        }
  ...
  const handleWizardAssessmentCreated = async (assessmentId: string) => {
    const sectionId = assignmentWizardSectionId;
    if (!sectionId || attachingItem) return;

    setAssignmentWizardSectionId(null);
    setAttachingItem(true);
    try {
      await moduleService.attachItem(sectionId, {
        itemType: "assessment",
        assessmentId,
        isGiven: false,
      });
      ...
      router.push(
        `/dashboard/teacher/assessments/${assessmentId}/edit?created=1`,
      );
    } catch {
      await fetchData();
      setAttachState({
        ...INITIAL_ATTACH_STATE,
        open: true,
        sectionId,
        itemType: "assessment",
      ...
      toast.error(
        "Assessment created, but not attached. Try Add Block again to attach the existing draft.",
      );
    } finally {
      setAttachingItem(false);
    }
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      ...
      setModule((current) =>
        current ? { ...current, teacherNotes: safeNotes } : current,
      );
  ...
  return (
    <div className="teacher-module-detail">
        ...
        <div className="teacher-module-detail__hero-row">
          ...
          <div className="teacher-module-detail__hero-copy">
            ...
            <div className="teacher-module-detail__hero-meta">
              ...
              <span>
                {lessonCount} lessons - {assessmentCount} assessments -{" "}
                {formatScheduleSummary(classItem)}
              </span>
            </div>
          </div>
        </div>
      ...
      <nav
        className="teacher-module-detail__tabs"
      ...
      <section className="teacher-module-detail__content">
            ...
            <div className="teacher-module-detail__section-head">
              ...
              {isCoreModule ? (
                <div className="teacher-module-detail__tip" data-tone="warning">
                  <strong>Default module:</strong> Section structure is
                  inherited from the template and cannot be edited here.
                </div>
              ) : (
                ...
                >
                  <header className="teacher-module-detail__section-card-head">
                      ...
                      {editingSectionId === section.id ? (
                        <div className="teacher-module-detail__section-edit">
                          ...
                          <Button
                            ...
                            data-priority="primary"
                            onClick={() =>
                              void handleSaveSectionTitle(section.id)
                            }
                    ...
                    <div className="teacher-module-detail__section-actions">
                      ...
                      <ActionTooltip
                        label={expanded ? "Collapse items" : "Expand items"}
                      >
                        <button
                          type="button"
                          className="teacher-module-detail__ghost"
                    ...
                    <div className="teacher-module-detail__items">
                      {section.items.length === 0 ? (
                        ...
                        section.items.map((item) => {
                          ...
                          const itemEditorHref = getItemEditorHref(
                            item,
                            classId,
                            moduleId,
                          );
                          ...
                          return (
                            <div
                              ...
                              onDragOver={(event) => event.preventDefault()}
                              onDrop={() =>
                                void handleReorderItems(section.id, item.id)
                              }
                            >
                                ...
                                {isCoreItem ? (
                                  <>
                                    <button
                                      ...
                                      onClick={() =>
                                        void handleOpenCoreItemPreview(item)
                                      }
                                      disabled={pending}
                                    ...
                                    {item.itemType === "assessment" ? (
                                      <label className="teacher-module-detail__control-toggle">
                                        <input
                                          ...
                                          disabled={
                                            pending ||
                                            isDraftAssessmentItem(item)
                                          }
                                          onChange={(event) =>
                                            void handleReleaseCoreItem(
                                              section.id,
                                              item.id,
                                              { isGiven: event.target.checked },
                                            )
                                          }
                                        />
                                        Give
                  ...
                  <footer className="teacher-module-detail__section-footer">
                    {isCoreModule ? (
                      <div
                        className="teacher-module-detail__tip"
                        data-tone="warning"
                      >
                        <strong>Default module:</strong> Add or remove blocks in
                        the template workspace, then create a fresh class from
                        that template.
                      </div>
                    ) : (
                      <button
                      ...
                      >
                        <span
                          className="teacher-module-detail__section-add-icon"
                          aria-hidden="true"
                        >
                          <Plus className="h-4 w-4" />
                        </span>
        ...
        {activeTab === "visibility" ? (
          <div className="teacher-module-detail__stack" data-animate="fade">
            ...
            <div className="teacher-module-detail__tip" data-tone="warning">
              <strong>Note:</strong> Hiding a module does not delete any
              content.
            </div>
          </div>
        ) : null}
        ...
        {activeTab === "locking" ? (
          <div className="teacher-module-detail__stack" data-animate="fade">
            ...
            {isCoreModule ? (
              <>
                <p className="teacher-module-detail__lead">
                  Template modules keep their copied structure, but you can
                  still lock or unlock student access here.
                </p>
                <div className="teacher-module-detail__tip" data-tone="warning">
                  <strong>Default module:</strong> Locking here changes student
                  access only. Edit the template itself if the structure needs
                  to change.
                </div>
                <div className="teacher-module-detail__choice-grid">
                  <button
                    ...
                    onClick={() =>
                      void handleReleaseCoreModule({ isLocked: false })
                    }
                    disabled={updatingModule}
                  >
                    ...
                    <div>
                      <h3>Unlocked</h3>
                      <p>
                        Students can open this template module once it is
                        visible and its items are given.
                      </p>
                    </div>
                    {!module.isLocked ? (
                      <span>
                  ...
                  <button
                    ...
                    data-active={module.isLocked}
                    onClick={() =>
                      void handleReleaseCoreModule({ isLocked: true })
                    }
                    disabled={updatingModule}
                  >
                    ...
                    <div>
                      <h3>Locked</h3>
                      <p>
                        Students can still see the module shell, but they cannot
                        open its content yet.
                      </p>
                    </div>
                    {module.isLocked ? (
                      <span>
              ...
              <>
                <p className="teacher-module-detail__lead">
                  Lock this module to prevent students from opening content
                  until you unlock it.
                </p>
                <div className="teacher-module-detail__choice-grid">
                  <button
                  ...
                  >
                    ...
                    <div>
                      <h3>Unlocked</h3>
                      <p>
                        Students can access all lessons and assessments in this
                        module.
                      </p>
                    </div>
                    {!module.isLocked ? (
                  ...
                  >
                    ...
                    <div>
                      <h3>Locked</h3>
                      <p>
                        Students see the module but cannot open lessons or
                        assessments.
                      </p>
                    </div>
                    {module.isLocked ? (
                ...
                <div className="teacher-module-detail__tip" data-tone="info">
                  <strong>Tip:</strong> Use locking to release modules
                  progressively.
                </div>
              </>
            )}
        ...
        {activeTab === "notes" ? (
          <div className="teacher-module-detail__stack" data-animate="fade">
            <h2>Module Notes</h2>
            <p className="teacher-module-detail__lead">
              Private notes visible only to you. Use this for reminders and
              pacing notes.
            </p>
          ...
          <DialogHeader>
            <DialogTitle>
              {previewLesson?.title ||
                ...
                (previewItem ? titleForItem(previewItem) : "Content Preview")}
            </DialogTitle>
            <DialogDescription>
        ...
        <DialogContent className="teacher-module-detail__attach-modal">
          ...
          <div className="teacher-module-detail__attach-modal-body">
            <div className="teacher-module-detail__attach-type-grid">
              {ATTACH_BLOCK_OPTIONS.map((option) => {
                ...
                return (
                  ...
                  >
                    <span
                      className="teacher-module-detail__attach-type-icon"
                      aria-hidden="true"
                    >
                      <Icon className="h-4 w-4" />
                    </span>
            ...
            {attachState.itemType === "file" ? (
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          setAttachState((current) => ({
                            ...current,
                            itemId:
                              option.value === "upload" ? "" : current.itemId,
                            file: null,
                            selectedLibraryFile:
                              option.value === "upload"
                                ? null
                                : current.selectedLibraryFile,
                          }));
                ...
                {attachSource === "upload" ? (
                  <>
                    ...
                    {attachState.file ? (
                      ...
                      <p className="teacher-module-detail__attach-note">
                        Upload a PDF or PowerPoint file to continue.
                      </p>
                    )}
                  </>
                ) : (
                  <>
                    ...
                    {attachState.itemId ? (
                      <p className="teacher-module-detail__attach-note">
                        Library file selected. Add block to attach it to this
                        section.
                      </p>
                    ) : (
                      <p className="teacher-module-detail__attach-note">
                        Choose a library file to continue.
                      </p>
                    )}
                  </>
                )}
              ...
              <>
                {attachState.itemType === "lesson" ? (
                  <div className="teacher-module-detail__attach-field">
                    <label htmlFor="attach-lesson-points">
                    ...
                    <p className="teacher-module-detail__attach-note">
                      A new empty draft lesson will be created and attached to
                      this section.
                    </p>
                  </div>
                ) : null}
                {attachState.itemType === "assessment" ? (
                  <>
                      ...
                      {ASSESSMENT_ATTACH_MODE_OPTIONS.map((option) => {
                        ...
                        return (
                          <button
                            ...
                            className="teacher-module-detail__attach-type"
                            data-active={
                              attachState.assessmentMode === option.value
                            }
                    ...
                    {attachState.assessmentMode === "create-new" ? (
                      <p className="teacher-module-detail__attach-note">
                        Continue to the guided basic settings for format,
                        class-record placement, name, and schedule.
                      </p>
                    ) : null}

                    {attachState.assessmentMode === "attach-existing" ? (
                      <>
                        <label htmlFor="attach-item">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                !canSubmitAttach || attachingItem || !attachState.itemType
              }
            >
              {attachingItem
                ? "Attaching..."
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              core controls for this module page.
            </DialogDescription>
          </DialogHeader>
          ...
          >
            <span>
              Page {helpPage + 1} of {teacherModuleGuidePages.length}
            </span>
            <div>
            ...
            <div className="teacher-intervention-workspace__manual-actions">
              ...
              {helpPage < teacherModuleGuidePages.length - 1 ? (
                <Button
                  ...
                  onClick={() =>
                    setHelpPage((current) =>
                      Math.min(current + 1, teacherModuleGuidePages.length - 1),
                    )
                  }
                >
                  Next page
      ...
      {assignmentWizardSectionId && user?.id ? (
        <NewAssignmentWizard
          classId={classId}
          actorId={user.id}
          onClose={() => setAssignmentWizardSectionId(null)}
          onCreated={(assessmentId) =>
            void handleWizardAssessmentCreated(assessmentId)
          }
        />
      ) : null}
      ...
      <LibraryFilePickerDialog
        ...
        subjectKey={normalizeLibrarySubjectKey(
