...
jest.mock("@/components/teacher/assessment/NewAssignmentWizard", () => ({
  NewAssignmentWizard: ({
    classId,
    actorId,
    onClose,
    ...
    <div role="dialog" aria-label="Assignment basic settings">
      <h2>Create an assignment</h2>
      <span>{classId}</span>
      <span>{actorId}</span>
      <button type="button" onClick={onClose}>
        Close assignment setup
      </button>
      <button type="button" onClick={() => onCreated("assessment-new")}>
...
describe("TeacherModuleDetailPage", () => {
  ...
  it("creates and attaches a new lesson block then opens lesson editor", async () => {
    ...
    const lessonTypeButton = within(dialog)
      .getByText("Lesson")
      .closest("button");
    if (!lessonTypeButton) {
  ...
  it("renders default-core module content as immutable with release controls", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Release Module" }),
    ).toBeInTheDocument();
  });
  ...
  it("allows unlocking a default module from the locking tab", async () => {
    ...
    await waitFor(() => {
      expect(mockedModuleService.releaseCoreModule).toHaveBeenCalledWith(
        "module-1",
        {
          isLocked: false,
        },
  ...
  it("opens the guided assignment settings before creating and attaching a new assessment block", async () => {
    render(<TeacherModuleDetailPage />);
    ...
    fireEvent.click(screen.getAllByRole("button", { name: "Add Block" })[0]);
    const dialog = await screen.findByRole("dialog");
    const assessmentTypeButton = within(dialog)
    ...
    fireEvent.click(
      within(dialog).getByRole("button", { name: "Open Basic Settings" }),
    );

    const setup = await screen.findByRole("dialog", {
      name: "Assignment basic settings",
    });
    expect(within(setup).getByText("Create an assignment")).toBeInTheDocument();
    expect(within(setup).getByText("teacher-1")).toBeInTheDocument();
    expect(mockedAssessmentService.createDraft).not.toHaveBeenCalled();

    fireEvent.click(
      within(setup).getByRole("button", { name: "Complete basic settings" }),
    );
    ...
    expect(pushMock).toHaveBeenCalledWith(
      "/dashboard/teacher/assessments/assessment-new/edit?created=1",
    );
  });

  it("recovers a wizard-created assessment as an existing attachment when section attachment fails", async () => {
    mockedModuleService.attachItem.mockRejectedValueOnce(
      new Error("attach failed"),
    );
    mockedAssessmentService.getByClass
    ...
    fireEvent.click(screen.getAllByRole("button", { name: "Add Block" })[0]);
    const dialog = await screen.findByRole("dialog");
    fireEvent.click(within(dialog).getByText("Assessment").closest("button")!);
    fireEvent.click(
      within(dialog).getByRole("button", { name: "Open Basic Settings" }),
    );
    const setup = await screen.findByRole("dialog", {
    ...
    await waitFor(() => {
      expect(
        within(recoveryDialog).getByLabelText("Available assessments"),
      ).toHaveValue("assessment-new");
    });
    expect(mockedAssessmentService.createDraft).not.toHaveBeenCalled();
  ...
  it("attaches an existing assessment without creating a new one", async () => {
    ...
    fireEvent.click(
      within(dialog).getByRole("button", {
        name: /Attach Existing Assessment/i,
      }),
    );
  ...
  it("updates assessment submit state when switching between create and existing modes", async () => {
    ...
    expect(
      within(dialog).getByRole("button", { name: "Add Block" }),
    ).toBeDisabled();
  ...
  it("attaches an existing library file instead of uploading a new pdf", async () => {
    ...
    fireEvent.click(screen.getAllByRole("button", { name: "Add Block" })[0]);
    const dialog = await screen.findByRole("dialog");
    const fileTypeButton = within(dialog)
      .getByText("PDF / PPTX")
      .closest("button");
    if (!fileTypeButton) {
    ...
    fireEvent.click(
      within(dialog).getByRole("button", { name: /Choose from Library/i }),
    );
  ...
  it("clears file attach state so the same library file can be attached again", async () => {
    ...
    await waitFor(() =>
      expect(mockedModuleService.attachItem).toHaveBeenCalledTimes(1),
    );
    await waitFor(() =>
  ...
  it("opens the module guide and navigates through all pages", async () => {
    ...
    expect(
      await screen.findByText("Teacher guide: Module Workspace"),
    ).toBeInTheDocument();
    expect(screen.getByText("Page 1 of 6")).toBeInTheDocument();
    expect(
      screen.getByText("Start from the module header"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText("Attach and manage module blocks"),
    ).toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: "Next page" }));
    expect(screen.getByText("Page 4 of 6")).toBeInTheDocument();
    expect(
      screen.getByText("Choose whether students can see this module"),
    ).toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: "Next page" }));
    expect(screen.getByText("Page 5 of 6")).toBeInTheDocument();
    expect(
      screen.getByText("Control release through locking"),
    ).toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: "Next page" }));
    expect(screen.getByText("Page 6 of 6")).toBeInTheDocument();
    expect(
      screen.getByText("Use private module notes for pacing"),
    ).toBeInTheDocument();
    ...
    await waitFor(() => {
      expect(
        screen.queryByText("Teacher guide: Module Workspace"),
