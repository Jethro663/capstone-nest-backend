...
describe('AssessmentHistoryWorklist', () => {
  ...
  it('defaults to a ten-row needs-attention page ordered by urgency', () => {
    ...
    expect(screen.getByRole('tab', { name: /Needs attention 13/i })).toHaveAttribute(
      'aria-selected',
      'true',
    );
    ...
    expect(screen.queryByText('Newest finished assessment')).not.toBeInTheDocument();

    const rows = screen.getAllByRole('row').slice(1);
    expect(within(rows[0]).getByText('Pending assessment 1')).toBeInTheDocument();
  ...
  it('changes views, resets pagination, and sorts finished work newest first', () => {
    ...
    const rows = screen.getAllByRole('row').slice(1);
    expect(within(rows[0]).getByText('Newest finished assessment')).toBeInTheDocument();
    expect(within(rows[0]).getByText('9/10')).toBeInTheDocument();
    expect(
      within(rows[0]).getByRole('link', { name: 'Open assessment' }),
    ).toHaveAttribute(
      'href',
      '/dashboard/teacher/assessments/finished-2',
    );
