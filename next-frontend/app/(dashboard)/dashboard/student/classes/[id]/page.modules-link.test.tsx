...
describe("StudentClassDetailPage module links", () => {
  ...
  it("routes module card open link to student module detail page", async () => {
    ...
    const bodyLink = await screen.findByRole("link", {
      name: "Open Module One module",
    });
  ...
  it("does not render a fallback description when a module has no description", async () => {
    ...
    expect(
      screen.queryByText(
        "Extended learning and higher-order thinking activities.",
      ),
  ...
  it("renders safe fixed copy when the required class request fails", async () => {
    ...
    expect(
      await screen.findByText("Class couldn't be loaded"),
    ).toBeInTheDocument();
    expect(
    ...
    ).toHaveAttribute("href", "/dashboard/student/courses");
  });
  ...
  it("renders safe forbidden copy without the server response body", async () => {
    ...
    expect(
      screen.queryByText("student enrollment join detail"),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: /back to courses/i }),
    ).toBeInTheDocument();
  ...
  it("keeps the class workspace visible when one secondary region fails", async () => {
    ...
    expect(
      screen.queryByText("No modules available yet."),
    ).not.toBeInTheDocument();
  ...
  it("retries only failed secondary regions without refetching the class owner", async () => {
    ...
    await screen.findByText("Class content is partially unavailable");
    fireEvent.click(
      screen.getByRole("button", { name: /retry failed content/i }),
    );
  ...
  it("renders manual and backend-gated copied template assignments returned by the API", async () => {
    ...
    expect(
      screen.getByRole("link", { name: /Manual Essay/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: /Given Core Quiz/i }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("link", { name: "Take" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("does not render JA as an embedded class surface", async () => {
    ...
    expect(
      screen.queryByText("Study support for this class"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("link", { name: /Ask JA/i }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("link", { name: /Practice with JA/i }),
    ).not.toBeInTheDocument();
    expect(
  ...
  it("supports module card and long-card view switch with persistence", async () => {
    ...
    const wideButton = await screen.findByRole("button", {
      name: "Wide Card View",
    });
    ...
    expect(
      window.localStorage.getItem("nexora.student.class.modules.view.class-1"),
    ).toBe("card");
  });
  ...
  it("opens the class page guide, walks through each page, and closes it", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: /class page help/i }),
    );

    expect(
      await screen.findByText("Student guide: Class Page"),
    ).toBeInTheDocument();
    expect(screen.getByText("Page 1 of 7")).toBeInTheDocument();
    expect(
      screen.getByText("Start here on the class page"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText("Open modules to study step by step"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText("Use assignments to find class work fast"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText("Check updates and join class discussions"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText("Use classmates when you need names and section info"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText("Read assessment grades without double counting"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText("Use the class calendar for due dates and events"),
    ).toBeInTheDocument();
    ...
    await waitFor(() => {
      expect(
        screen.queryByText("Student guide: Class Page"),
      ).not.toBeInTheDocument();
    });
  });
  ...
  it("renders the discussion thread feed with attachments, reactions, and plain reply composer", async () => {
        ...
        items: [
          {
            ...
            bodyHtml:
              "<p>Share what surprised you in today&apos;s activity.</p>",
            themeId: "default",
            commentLimitPerStudent: null,
            allowComments: true,
            isPinned: true,
    ...
    fireEvent.change(
      screen.getByPlaceholderText("Write a respectful reply..."),
      {
  ...
  it("disables the reply composer when the student reached the thread comment limit", async () => {
    ...
    expect(
      await screen.findByText("Comment limit reached"),
    ).toBeInTheDocument();
    expect(
      screen.getByPlaceholderText("Write a respectful reply..."),
    ).toBeDisabled();
  ...
  it("refetches the open discussion thread when a matching realtime notification arrives", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: /open thread/i }),
    );
    ...
    await waitFor(
      () => {
        ...
        expect(mockedDiscussionBoardService.listThreads).toHaveBeenCalledTimes(
          1,
        );
      },
      { timeout: 4000 },
