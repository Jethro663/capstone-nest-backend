...
export function buildSchoolYearList(
  ...
  const values = new Set<string>([getCurrentSchoolYearReference(now)]);

  for (const classItem of classes) {
    if (classItem.schoolYear) values.add(classItem.schoolYear);
  }
  for (const schoolEvent of schoolEvents) {
    if (schoolEvent.schoolYear) values.add(schoolEvent.schoolYear);
  }

  return [...values].sort((left, right) => right.localeCompare(left));
}

function expandClassSchedulesForMonth(
  classes: ClassItem[],
  month: Date,
  selectedSchoolYear: string,
): CalendarFeedItem[] {
...
export function normalizeCalendarFeed({
  ...
  items.push(...expandClassSchedulesForMonth(filteredClasses, month, selectedSchoolYear));
