...
WRITE_PACKET_TERMS = {
    "implement", "implementation", "implementer", "edit", "modify", "write",
    "patch", "fix", "refactor", "delete", "runtime-affecting",
}
READ_ONLY_PACKET_TERMS = {"research", "audit", "review", "verification", "browser_qa", "read-only", "do not edit"}
EXPLICIT_READ_ONLY_PATTERNS = [
    re.compile(r"\bdo\s+not\s+(?:edit|modify|write|patch|delete|change)\b"),
    re.compile(r"\bno\s+(?:edits?|modifications?|writes?|patches?|changes?)\b"),
    re.compile(r"\bread[-\s]?only\b"),
    re.compile(r"\bwithout\s+(?:editing|modifying|writing|patching|changing|deleting)\b"),
]
WRITE_PACKET_RX = re.compile(
    r"\b(?:implement(?:ation|er|ing|ed)?|edit(?:ing|ed|s)?|modif(?:y|ies|ied|ying)|"
    r"writ(?:e|es|ing|ten)|patch(?:ing|ed|es)?|fix(?:ing|ed|es)?|"
    r"refactor(?:ing|ed|s)?|delet(?:e|es|ed|ing)|runtime-affecting)\b"
)
...
def requires_strict_context(data: dict[str, Any]) -> bool:
    blob = " ".join(
        str(data.get(k, ""))
        for k in ["role", "objective", "scope", "focus", "tasks"]
    ).lower()
    if not blob:
        return False
    for pattern in EXPLICIT_READ_ONLY_PATTERNS:
        blob = pattern.sub(" ", blob)
    return bool(WRITE_PACKET_RX.search(blob))
...
def main() -> None:
    ...
    ap.add_argument("--out", help="Optional path to write the Dispatch Packet")
    ...
    ap.add_argument("--no-strict", action="store_true", help="Do not auto-enforce scoped must-read context for implementation/edit packets")
    ...
    if strict_required and not split_items(data.get("must_read")):
        print("Strict Dispatch Packet requires at least one scoped --must-read item for implementation/edit packets.", file=sys.stderr)
        raise SystemExit(2)
    ...
    if emit_event is not None and args.run_id:
        emit_event(Path(args.root), event="dispatch_compiled", run_id=args.run_id, status="ready", phase_id=args.phase_id, agent=args.agent or data.get("role"), summary=data.get("objective", "Dispatch Packet compiled"), metadata={"chars": len(text), "out": args.out or "stdout"}, update_state=True)
