...
def validate(required: list[str], handoff: str, require_explicit: bool = True) -> list[str]:
    problems: list[str] = []
    fields = parse_structured_packet(handoff)
    lower = handoff.lower()
    coverage = field_text(fields, ["context_coverage", "context coverage"]) if fields else ""
    if require_explicit and not coverage and "context" not in lower:
    ...
    coverage_source = "\n".join(
        x for x in [
            coverage,
            field_text(fields, ["files_read", "files read", "files_touched_or_examined", "files touched inspected"]),
        ] if x
    )
    normalized_text = normalize_path(coverage_source or handoff)
    ...
    structured_or_text = coverage or handoff
    if re.search(r"(?i)safe_to_modify\s*:\s*true", structured_or_text) and missing:
        problems.append("Handoff claims safe_to_modify=true while required context is missing")
    if re.search(r"(?i)missing_context\s*:\s*(\[\s*\]|none|\[?\s*none\s*\]?)", structured_or_text) and missing:
...
def main() -> None:
    ...
    if emit_event is not None and args.run_id:
        emit_event(Path(args.root), event="context_coverage_passed" if result["status"] == "OKAY" else "context_coverage_failed", run_id=args.run_id, status="passed" if result["status"] == "OKAY" else "failed", phase_id=args.phase_id, agent=args.agent, summary=("context_coverage_passed" if result["status"] == "OKAY" else "; ".join(result.get("problems", [])[:2])), metadata={"problem_count": len(result.get("problems", []))}, update_state=True)
    if args.json:
