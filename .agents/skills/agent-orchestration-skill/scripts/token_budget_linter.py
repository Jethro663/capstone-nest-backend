...
def main() -> None:
    ...
    # Production layout guard: this project intentionally uses `skills/`, not `.skills`,
    # and `subagents/`, not `.codex/agents`. Hidden legacy layouts are backed up by
    # the installer to prevent duplicate skill discovery and wrong npm payloads.
    if (root / ".skills").exists():
        problems.append("Legacy hidden .skills directory is present; use skills/ without a leading dot")
    if (root / ".agents" / "skills" / "agent-orchestration-skill").exists():
        problems.append("Legacy .agents/skills install is present; use skills/agent-orchestration-skill")
    ...
    agents_dir = root / "subagents"
    agents = [p for p in sorted(agents_dir.glob("*.toml")) if p.name != "config.toml"] if agents_dir.exists() else []
    ...
    for wrapper in ["agentic-orchestration-control", "aoc"]:
        if not (skill_root / "bin" / wrapper).exists():
            problems.append(f"Missing control-room command wrapper: {wrapper}")
    ...
    if master_skill.exists():
        master_text = master_skill.read_text(encoding="utf-8", errors="replace")
        ...
        problems.append("Missing master skill: skills/agent-orchestration-skill/SKILL.md")
