...
- [ ] 3.1 Implement the pure chat model for approved prompts, entry transitions, latest-thread selection, and smart lesson selection
- [ ] 3.2 Implement the preset-only JaChatWorkspace with one message FlatList and anchored prompt launcher
- [ ] 3.3 Implement native menu, prompt, lesson, and lazy activity-history sheets with Android Back dismissal
- [ ] 3.4 Add component tests for sheets, prompt submission, busy state, empty/stale states, blocked replies, citations, and errors

## 4. JAHUB screen integration

- [ ] 4.1 Render Ask directly outside ScreenScroll while preserving current Replay and Learner's Path layouts and calls
- [ ] 4.2 Integrate latest-thread resume, New Chat suppression, smart context selection, context-change confirmation, refresh, and unchanged prompt payloads
- [ ] 4.3 Update JaScreen integration tests for class changes, secondary panels, stale context, and preset-only behavior
- [ ] 4.4 Run focused JAHUB tests and typecheck, then commit the JAHUB slice independently

## 5. Regression and device gates

- [ ] 5.1 Run the complete mobile Jest suite, typecheck, and git diff check with no new touched-file warnings
- [ ] 5.2 Run an Expo production export and Android native build with an explicit non-localhost API URL
