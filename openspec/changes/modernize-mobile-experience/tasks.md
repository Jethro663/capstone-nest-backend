...
- [x] 2.3 Adapt teacher, student, and admin presentation wrappers to the shared system without changing their domain-facing behavior.
- [x] 2.4 Replace remaining record-filter button rows with the shared filter selector while retaining chips only for selection, tagging, and non-filter toggles.
...
- [x] 5.3 Add failing tests for the scannable assessment overview, submission selector, semantic scores, and interactive analytics question details.
- [x] 5.4 Implement the assessment detail hierarchy and analytics drill-down using only existing server evidence.
...
- [x] 8.4 Commit and push the verified exact SHA, observe CI/deployment and public artifact evidence, and record any remaining physical-device acceptance gate.
