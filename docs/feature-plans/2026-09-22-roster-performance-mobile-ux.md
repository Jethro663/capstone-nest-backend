...
- Unverified now: actual cause of class-card hit failure; actual data distributions and analytics semantics in a school account; behavior on the reporter's physical Android device; whether local browser access can authenticate to teacher/admin routes. Resolve these during implementation/testing, and do not claim device or live acceptance without evidence.
...
Release gate still open: a new production APK must be signed with the existing Nexora release key, then verified and registered in the download/update manifest. The four `NEXORA_RELEASE_*` signing properties are absent in this session; the only keystore discovered locally is `debug.keystore`. Do not publish a debug-signed or old APK as the requested update. The user has been asked for the secure location of the existing production signing setup, without sending credentials in chat. Until signing is available, do not bump the public manifest, push a partial release, or claim deployment/physical-device acceptance.
